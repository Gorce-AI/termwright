...
function renderSkillMarkdown(): string {
  return [
    ...
    '',
    'Set `record: true` at launch to keep a bounded trace of a manual exploration. The recording',
    'is opt-in because the archive retains terminal output and input by default. Set',
    '`record: { redact: true }` at launch before recording',
    'an artifact that may leave the project. Command arguments are always withheld.',
    '`terminal.close` returns `tracePath` after it',
    'finalizes the archive. Open that path with `trace.open` to replay the session.',
    '',
    '## Reading a snapshot',
    '',
    '```',
    ...
    '',
    'If a composite control is covered by its child, `terminal.click_at { terminal, point:',
    '{ row, column } }` sends a physical click without claiming semantic ownership. The child',
    'may stop propagation, so wait for the application result after the click.',
    '',
    '`terminal.query` reads the current state and returns zero matches immediately. Use',
    '`terminal.wait_for` for an element or state to appear. A successful `terminal.click` confirms',
    'that mouse input was sent; wait for the application postcondition separately.',
    '',
    'For application selection, send real mouse input with `terminal.drag { terminal,',
    'from: {row, column}, to: {row, column} }`, then `terminal.press` for Ctrl+C and',
    '`terminal.wait_for` for the resulting state. `terminal.select_cells` and',
    '`terminal.copy_selection` operate on Termwright emulator cells only and cannot test',
    'application copy behavior.',
    '',
    ...
    'The `redactions` field lists exact masked cells and source node IDs. When a modal covers a',
    'sensitive field, public text is left visible only if the current cell grid and authoritative',
    'hit map prove that the modal owns those cells.',
    '',
