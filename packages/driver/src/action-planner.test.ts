...
function context(
  ...
  const node = {
    ...
    inputRecipes: [
      ...
      { action: 'activate', requiresFocus: true, steps: [{ kind: 'press', key: 'Enter' }] },
      {
        action: 'setValue',
        requiresFocus: true,
        steps: [{ kind: 'press', key: 'Control+U' }, { kind: 'insert-action-value' }],
      },
    ],
    geometry: {
      ...
      intendedRect: {
        status: 'known',
        value: { row: 2, column: 3, width: 6, height: 1 },
        evidence,
      },
  ...
  return {
    ...
    hitGrid: () => ({
      status: 'known',
      value: {
        regions: [{ recipientId: node.id, rect: { row: 2, column: 3, width: 6, height: 1 } }],
      },
      evidence,
    }),
...
const target = Object.freeze({
  ref: 'semantic:field@3',
  semantic: true,
  identity: 'stable' as const,
  revision: 3,
  rect: { row: 2, column: 3, width: 6, height: 1 },
  role: 'textbox',
  name: 'Name',
  state: { focused: true },
});
...
describe('canonical condition evaluation', () => {
  it('does not turn unknown or unsupported into success through negation', () => {
    ...
    expect(
      evaluateCondition(condition, stamp, () => ({ status: 'unknown', reason: 'provider-refresh' }))
        .verdict,
    ).toBe('inconclusive');
    expect(
      evaluateCondition(condition, stamp, () => ({
        status: 'unsupported',
...
describe('ActionPlanner keyboard strategies', () => {
  it.each(['parser-in-flight', 'semantic-frame-open', 'pairing-pending'] as const)(
    ...
    (state) => {
      ...
      expect(() =>
        planner.planKeyboard('pending', { kind: 'activate', targetRef: target.ref }, target),
      ).toThrow(
      ...
      expect(() =>
        planner.planPointer('pending-pointer', { kind: 'click', targetRef: target.ref }, target),
      ).toThrow(
        expect.objectContaining({
          code: 'stale-snapshot',
          message: expect.stringContaining(state),
  ...
  it('rejects unfocused type without planning hidden pointer input', () => {
    ...
    expect(
      planner.explain({ kind: 'type', targetRef: target.ref }, target, undefined, 'Ada'),
    ).toMatchObject({
      actionable: false,
      reason: { code: 'not-actionable' },
    });
  ...
  it('plans fill from an authoritative focus and replace-value recipe at one checkpoint', () => {
    const plan = new ActionPlanner(context({ state: { focused: false } })).planKeyboard(
      'a2',
      { kind: 'fill', targetRef: target.ref },
      target,
      'Ada',
    );
  ...
  it('makes check idempotent and rejects non-checkable targets', () => {
    const checkbox = {
      ...
      state: { focused: true, checked: true },
    };
    const planner = new ActionPlanner(
      context({ role: 'checkbox', state: { focused: true, checked: true } }),
    );
    expect(
      planner.planKeyboard('a4', { kind: 'check', targetRef: target.ref }, checkbox).operations,
    ).toEqual([]);
    expect(() =>
      new ActionPlanner(context()).planKeyboard(
        'a5',
        { kind: 'check', targetRef: target.ref },
        target,
      ),
...
describe('ActionPlanner runtime input requirements', () => {
  it('allows hover over a disabled target without weakening click actionability', () => {
    ...
    const hover = planner.planPointer(
      'disabled-hover',
      { kind: 'hover', targetRef: target.ref },
      target,
    );
    ...
    expect(hover.plan.requirements.some(({ condition }) => condition.kind === 'enabled')).toBe(
      false,
    );
    expect(() =>
      planner.planPointer('disabled-click', { kind: 'click', targetRef: target.ref }, target),
    ).toThrowError(expect.objectContaining({ code: 'not-actionable' }));
  });
  ...
  it('allows an unrelated newer screen revision when the target region is unchanged', () => {
    ...
    expect(
      planner.planPointer('stable-local', { kind: 'click', targetRef: target.ref }, target).plan
        .checkpoint,
    ).toBe(newer);
  });
  ...
  it('rejects a newer screen revision that damaged the target region', () => {
    ...
    expect(() =>
      planner.planPointer('damaged-local', { kind: 'click', targetRef: target.ref }, target),
  ...
  it('acts through a repaint that left the target where the tree says it is', () => {
    ...
    expect(
      planner.planPointer('repainted', { kind: 'click', targetRef: target.ref }, target).plan
        .checkpoint,
    ).toBe(newer);
  });
  ...
  it('refuses when something else took the target’s place', () => {
    ...
    expect(() =>
      planner.planPointer('replaced', { kind: 'click', targetRef: target.ref }, target),
    ).toThrowError(expect.objectContaining({ code: 'stale-snapshot' }));
  });
  ...
  it('still refuses when the coordinate system moved, whatever the region reads', () => {
    ...
    expect(() =>
      planner.planPointer('scrolled', { kind: 'click', targetRef: target.ref }, target),
    ).toThrowError(expect.objectContaining({ code: 'stale-snapshot' }));
  });
  ...
  it('accepts an explicit frozen application region contract without inventing a hit test', () => {
    ...
    const { plan } = planner.planPointer(
      'region-only',
      { kind: 'click', targetRef: target.ref },
      target,
    );
    expect(plan.physicalRegion?.spans).toEqual([{ row: 2, from: 3, to: 9 }]);
    expect(
      plan.requirements.find(({ condition }) => condition.kind === 'receives-pointer'),
  ...
  it('accepts authoritative production-router ownership when framework display state is unavailable', () => {
    ...
    expect(plan.requirements.find(({ condition }) => condition.kind === 'displayed')).toMatchObject(
  ...
  it('fails closed when semantic evidence cannot be paired with a terminal frame', () => {
    ...
    const unpairedContract = Object.freeze({
      ...
      capabilities: Object.freeze({
        ...baseContract.capabilities,
        'paired-revisions': Object.freeze({
          status: 'unsupported' as const,
          reason: 'not-negotiated' as const,
        }),
      }),
    ...
    expect(() =>
      planner.planPointer('unpaired-pointer', { kind: 'click', targetRef: target.ref }, target),
    ).toThrowError(expect.objectContaining({ code: 'capability-unavailable' }));
    expect(() =>
      planner.planKeyboard(
        'unpaired-keyboard',
        { kind: 'type', targetRef: target.ref },
        target,
        'x',
      ),
    ).toThrowError(expect.objectContaining({ code: 'capability-unavailable' }));
  ...
  it('treats an absent region as current non-actionability, not missing session support', () => {
    ...
    const explanation = (
      failure as {
        actionability?: { requirements: readonly { condition: Condition; verdict: string }[] };
      }
    ).actionability;
    expect(
      explanation?.requirements.find(({ condition }) => condition.kind === 'pointer-region'),
