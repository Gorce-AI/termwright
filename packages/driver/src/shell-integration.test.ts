...
describe("managed shell integration", () => {
  ...
  it("installs PowerShell integration through its startup command", () => {
    expect(
      integratedPowerShellCommand(["pwsh.exe", "-NoLogo", "-NoProfile"]),
    ).toEqual([
      "pwsh.exe",
      "-NoLogo",
      "-NoProfile",
  ...
  it("refuses every non-allowlisted startup argument, including legal aliases and abbreviations", () => {
    for (const option of [
      ...
      expect(() =>
        integratedPowerShellCommand(["pwsh.exe", option, "something"]),
      ).toThrow(TypeError);
    }
  });
  ...
  it("validates the arity of safe PowerShell startup options", () => {
    expect(() =>
      integratedPowerShellCommand(["pwsh.exe", "-ExecutionPolicy"]),
    ).toThrow(TypeError);
    expect(() =>
      integratedPowerShellCommand([
        "pwsh.exe",
  ...
  it("single-quotes a POSIX command inside eval", () => {
    expect(wrapPosixShellCommand("printf '%s' \"it's safe\"")).toContain(
      "it'\\''s safe",
    );
  });
