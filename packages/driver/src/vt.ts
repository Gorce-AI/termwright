...
export class VtScreen {
  ...
  modes(): TerminalModes {
    ...
    return Object.freeze({
      ...
      kittyKeyboardFlags: this.#kittyKeyboardStacks[this.activeBuffer()].at(-1) ?? 0,
  ...
  #registerHandlers(): void {
    ...
    // The pinned Termwright OpenConsole never uses standard DSR/CPR for its
    // own cursor shadow. Its request carries an unguessable per-request token,
    // so an application query cannot steal the host reply (or vice versa).
    // Read the xterm cursor here, at the exact parser position of the request.
    parser.registerOscHandler(CONPTY_HOST_CURSOR_OSC_CODE, (data: string) => {
      const request = parseConPtyHostCursorRequest(data);
      if (request === null) return false;
      const buffer = this.terminal.buffer.active;
      this.#emitResponse({
        data: encodeConPtyHostCursorResponse(request, buffer.cursorY + 1, buffer.cursorX + 1),
    ...
    const scalar = (value: number | number[] | undefined, fallback: number): number => {
      const candidate = Array.isArray(value) ? value[0] : value;
      return Number.isSafeInteger(candidate) && (candidate ?? -1) >= 0 ? candidate! : fallback;
    };
    const keyboardStack = (): number[] => this.#kittyKeyboardStacks[this.activeBuffer()];
    const setKeyboardFlags = (flags: number, mode: number): void => {
      const stack = keyboardStack();
      const current = stack.at(-1) ?? 0;
      const supported = flags & 31;
      stack[stack.length - 1] =
        mode === 2 ? current | supported : mode === 3 ? current & ~supported : supported;
    };
    // Kitty progressive keyboard protocol. The stack is buffer-local and its
    // query reply uses the same ordered terminal-response plane as DSR.
    parser.registerCsiHandler({ prefix: '?', final: 'u' }, () => {
      this.#emitResponse({
        data: `\x1b[?${keyboardStack().at(-1) ?? 0}u`,
        kind: 'keyboard-protocol',
      });
      return true;
    });
    parser.registerCsiHandler({ prefix: '>', final: 'u' }, (params) => {
      const stack = keyboardStack();
      const flags = scalar(params[0], 0) & 31;
      if (stack.length >= 32) stack[stack.length - 1] = flags;
      else stack.push(flags);
      return true;
    });
    parser.registerCsiHandler({ prefix: '<', final: 'u' }, (params) => {
      const stack = keyboardStack();
      const count = Math.max(1, scalar(params[0], 1));
      for (let index = 0; index < count && stack.length > 1; index += 1) stack.pop();
      return true;
    });
    parser.registerCsiHandler({ prefix: '=', final: 'u' }, (params) => {
      setKeyboardFlags(scalar(params[0], 0), scalar(params[1], 1));
      return true;
    });
