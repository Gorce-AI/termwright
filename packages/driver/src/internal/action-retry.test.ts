...
import { AmbiguousLocatorError, CapabilityUnavailableError, NotActionableError, StaleSnapshotError } from '../errors.js';
import { ActionRetryController, assertBeforeActionInput } from './action-retry.js';

const diagnostics = { semanticTree: true } as const;
const stamp = (sequence: number): ObservationStamp => Object.freeze({
  sessionId: 's', contractId: 's:0', epoch: 0, sequence,
...
describe('locator action retry policy', () => {
  it('retries the first pre-input stale race immediately', async () => {
    ...
    await new ActionRetryController(1_000).retry(
      new StaleSnapshotError('race', diagnostics),
      { checkpoint: () => stamp(2), waitForChange },
    );
  ...
  it.each(['target-state', 'pointer-region', 'covered'] as const)(
    ...
    async (reason) => {
      let current = stamp(4);
      const waitForChange = vi.fn(async () => { current = stamp(5); });
  ...
  it('checks the monotonic deadline immediately after a wait', async () => {
    ...
    await expect(controller.retry(transient('target-state', 1), {
      checkpoint: () => stamp(1),
      waitForChange: async () => { now = 15; },
  ...
  it.each([
    ...
    await expect(new ActionRetryController(1_000).retry(failure, {
      checkpoint: () => stamp(1), waitForChange,
