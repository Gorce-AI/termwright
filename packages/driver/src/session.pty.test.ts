...
afterEach(async () => {
  ...
  const results = await Promise.allSettled(
    owned.map(async (terminal) => terminal.close()),
  );
...
describe.skipIf(!ptyAvailable())(
  "a generic session over a real PTY",
  { timeout: 20_000 },
  () => {
    ...
    it.skipIf(process.platform === "win32")(
      ...
      async () => {
        ...
        const unsubscribe = terminal.events.on("input", (event) =>
          inputs.push(event),
        );
        try {
    ...
    it("observes output, title and exit status", async () => {
      ...
      expect(
        (await terminal.settled()).capabilities["semantic-tree"].status,
      ).toBe("unsupported");
      ...
      expect(terminalState.modes).toEqual(
        expect.objectContaining({ bracketedPaste: false }),
      );
    ...
    it("names a launch path that does not exist instead of dying blank", async () => {
      ...
      expect((missingCwd as TermwrightError).message).toContain(
        "no-such-directory",
      );
    ...
    it("resizes the pty and the emulator together", async () => {
      ...
      expect(receipt.after.screenRevision).toBeGreaterThan(
        receipt.before.screenRevision,
      );
    ...
    it("reports a typed timeout with a screen excerpt", async () => {
      ...
      expect((error as TermwrightError).diagnostics.screenExcerpt).toContain(
        "READY",
      );
    });
  },
...
describe.skipIf(!ptyAvailable())("crash reports", { timeout: 20_000 }, () => {
  it("captures the stack trace of a program that threw", async () => {
    ...
    expect(inputs.map((input) => input.preview)).toEqual([
      undefined,
      undefined,
    ]);
  ...
  it("reports the crash from any wait that can no longer make progress", async () => {
    ...
    expect((error as TermwrightError).diagnostics.suggestion).toContain(
      "crashReport()",
    );
  });
  ...
  it("keeps the last semantic tree of an instrumented program that died", async () => {
    ...
    expect(report?.lastSemanticTree?.nodes.map((node) => node.name)).toContain(
      "Approve",
    );
    expect(
      report?.diagnosticsTail.some(
        (entry) => entry.code === "adapter-attached",
      ),
    ).toBe(true);
  });
...
describe.skipIf(!ptyAvailable())("action events", { timeout: 20_000 }, () => {
  it("reports every action in order, with the target it resolved", async () => {
    ...
    expect(actions.map((event) => event.api)).toEqual([
      "activate",
      "press",
      "resize",
    ]);
    expect(starts.map((event) => event.api)).toEqual([
      "activate",
      "press",
      "resize",
    ]);
    expect(starts.map((event) => event.actionId)).toEqual(
      actions.map((event) => event.actionId),
    );
    ...
    expect(activation?.receipt?.executed).toEqual(
      activation?.receipt?.plan.operations,
    );
  ...
  it("reports the action after it finished, not when it started", async () => {
    ...
    const order: string[] = [];
    terminal.events.on("action-start", (event) =>
      order.push(`start:${event.api}`),
    );
...
describe.skipIf(!ptyAvailable())("settled()", { timeout: 20_000 }, () => {
  it("resolves with the final verdict for a generic program", async () => {
    ...
    expect(capabilities.capabilities["semantic-tree"].status).toBe(
      "unsupported",
    );
    ...
    const error = await terminal
      ...
      .resolve({ timeout: 100 })
      .catch((cause: unknown) => cause as TermwrightError);
    expect((error as TermwrightError).code).toBe(
  ...
  it("uses the startup readiness budget when semantic capabilities are required", async () => {
    ...
    expect(terminal.contract()?.capabilities["semantic-tree"].status).toBe(
      "supported",
    );
    expect(
      terminal
    ...
    it("passes the whole environment through when asked explicitly", async () => {
      ...
      try {
        ...
        expect(terminal.screen().text()).toContain(
          "ENV TERMWRIGHT_FIXTURE_SECRET=shared-on-purpose",
        );
      } finally {
        delete process.env["TERMWRIGHT_FIXTURE_SECRET"];
      }
...
describe.skipIf(!ptyAvailable())(
  ...
  () => {
    it("uses only OSC 133 prompt marks as authoritative readiness", async () => {
      ...
      expect(terminal.diagnostics().at(-1)?.code).toBe(
      ...
      expect(
        terminal
          .diagnostics()
          .filter((entry) => entry.code === "ready-shell-integration"),
      ).toHaveLength(2);
    });
    ...
    it("exposes screen silence only through the explicit quiet heuristic", async () => {
      ...
      expect(
        terminal
          .diagnostics()
          .some((entry) => entry.code === "ready-shell-integration"),
      ).toBe(false);
    });
    ...
    it("does not call an exited program ready, even with a prompt on screen", async () => {
      ...
      const terminal = await launchWith([
        "prompt-app.mjs",
        "--exit-after-prompt",
      ]);
      await terminal.waitForExit();
...
describe.skipIf(!ptyAvailable())(
  ...
  () => {
    it("returns exact command boundaries, exit status, cwd and terminal state", async () => {
      ...
      expect(result.receipt.after.contractId).toBe(
        result.receipt.before.contractId,
      );
    ...
    it("does not infer shell support from a quiet generic program", async () => {
      const terminal = await launch("echo-app.mjs");
      await expect(
        terminal.shell.waitForPrompt({ timeout: 30 }),
  ...
  () => {
    it("records why a generic session stayed generic, and emits it", async () => {
      ...
      const entry = terminal
        .diagnostics()
        .find((item) => item.code === "negotiation-timeout");
    ...
    it("records which wire error closed the channel", async () => {
      ...
      await expect
        .poll(
          () =>
            terminal
              .diagnostics()
              .some((entry) => entry.code === "protocol-violation"),
          {
            timeout: 5_000,
          },
        )
      ...
      const violation = terminal
        .diagnostics()
        .find((entry) => entry.code === "protocol-violation");
    ...
    it("records the handshake and the advisory revision-commit", async () => {
      ...
      await expect
        ...
        .toEqual(
          expect.arrayContaining(["adapter-attached", "revision-commit"]),
        );

      const commit = terminal
        .diagnostics()
        .find((entry) => entry.code === "revision-commit");
...
describe.skipIf(!ptyAvailable())("locatorForRef", { timeout: 20_000 }, () => {
  ...
  it("round-trips a grid ref and rejects nonsense", async () => {
    ...
    expect(() =>
      terminal.locatorForRef("not-a-ref!" as import("./api.js").LocatorRef),
    ).toThrow(TypeError);
  });
});
...
describe.skipIf(!ptyAvailable())(
  ...
  () => {
    ...
    it("rejects zero, fractional, and huge locator wheel deltas before writing PTY bytes", async () => {
      ...
      await expect(locator.wheel({ deltaY: 0, deltaX: 0 })).rejects.toThrow(
        TypeError,
      );
    ...
    it("rejects horizontal locator wheel input under an unobservable mouse contract without bytes", async () => {
      ...
      await expect(
        terminal.getByScreenText("MOUSE ON").wheel({ deltaX: 1 }),
      ).rejects.toMatchObject({
        code: "capability-unavailable",
      });
      expect(input).toHaveLength(0);
    ...
    it("refuses a drag when the tracking level is insufficient or unobservable", async () => {
      ...
      const locatorOutcome = await terminal
        ...
        .catch((cause: unknown) => cause as TermwrightError);
      const pointerInput =
        terminal.contract()?.capabilities["pointer-input"].status;
      ...
      expect(input).toHaveLength(0);
      const failedLocatorAction = actions.find(
        (event) => event.api === "dragTo",
      );
    ...
    it("reports unavailable pointer capability when the backend hides mouse modes", async () => {
      ...
      expect((error as TermwrightError).message).toContain(
        "outside the effective session contract",
      );
    });
...
describe.skipIf(!ptyAvailable())(
  ...
  () => {
    ...
    it("re-resolves a ref when the probe has stable identity", async () => {
      ...
      expect(terminal.contract()?.capabilities["stable-identity"].status).toBe(
        "supported",
      );
    ...
    it("refuses to re-resolve a ref the framework cannot keep", async () => {
      ...
      expect((await terminal.getByTestId("approve").resolve()).name).toBe(
        "Approve",
      );
    });
    ...
    it("never turns framework-local geometry into physical pointer input", async () => {
      ...
      expect(
        terminal.contract()?.capabilities["intended-geometry"].status,
      ).toBe("unsupported");
...
describe.skipIf(!ptyAvailable())(
  "a semantic session over a real PTY",
  { timeout: 20_000 },
  () => {
    it("does not report hidden or detached before the first semantic revision commits", async () => {
      ...
      await expect(
        missing.waitFor({ state: "hidden", timeout: 60 }),
      ).rejects.toMatchObject({ code: "timeout" });
      await expect(
        missing.waitFor({ state: "detached", timeout: 60 }),
      ).rejects.toMatchObject({ code: "timeout" });
    ...
    it("negotiates the tree, pairs revisions and resolves semantic locators", async () => {
      ...
      expect(contract.capabilities["paired-revisions"].status).toBe(
        "supported",
      );
    ...
    it("supports Termwright semantic selectors, within() and testIds", async () => {
      ...
      expect(await terminal.getByTestId("approve").textContent()).toBe(
        "Approve",
      );
      expect(await terminal.locator("button:focused").textContent()).toBe(
        "Approve",
      );
    });
    ...
    it("composes semantic locators lazily with descendants, filters, boolean algebra and positional selection", async () => {
      ...
      expect(
        await dialog
          .filter({ has: terminal.getByRole("button", { name: "Reject" }) })
          .count(),
      ).toBe(1);
      expect(
        await dialog
          .filter({ hasNot: terminal.getByRole("button", { name: "Missing" }) })
          .count(),
      ).toBe(1);

      const approve = terminal
        .getByRole("button")
        .and(terminal.getByTestId("approve"));
      expect(await approve.textContent()).toBe("Approve");
      const either = terminal
        .getByTestId("approve")
        .or(terminal.getByTestId("reject"));
      ...
      expect(await buttons.nth(0).filter({ hasText: "Reject" }).count()).toBe(
        0,
      );
      expect(
        await buttons.filter({ hasText: "Reject" }).nth(0).textContent(),
      ).toBe("Reject");
      ...
      expect(() =>
        terminal
          .getByRole("button")
          .or(terminal.getByScreenText("Approve") as never),
      ).toThrow(/cannot combine semantic and terminal-grid/u);
      expect(() =>
        (
          dialog as unknown as { getByScreenText(text: string): unknown }
        ).getByScreenText("Approve"),
      ).toThrow(/cannot combine semantic and terminal-grid/u);
      ...
      expect(() =>
        terminal.getByRole("button").within(other.getByRole("dialog")),
    ...
    it.skipIf(process.platform === "win32")(
      ...
      async () => {
        ...
        const wire = Buffer.concat(
          input.map((bytes) => Buffer.from(bytes)),
        ).toString("utf8");
    ...
    it("still refuses semantic locators once the session is generic for good", async () => {
      ...
      const early = await terminal
        ...
        .resolve({ timeout: 50 })
        .catch((cause: unknown) => cause as TermwrightError);
      expect((early as TermwrightError).code).toBe(
        "semantic-capability-unavailable",
      );
    ...
    it("reports an empty published value as empty text, not as the label", async () => {
      ...
      expect(
        await terminal.getByTestId("approve").semanticValue(),
      ).toMatchObject({ status: "absent", reason: "no-value" });
    });
    ...
    it("fails closed when the negotiated semantic provider disappears", async () => {
      ...
      await expect
        .poll(() =>
          terminal
            .diagnostics()
            .some((entry) => entry.code === "adapter-disconnected"),
        )
        .toBe(true);
      expect(terminal.semanticTree()).toBe(retained);
      await expect(
        terminal.getByTestId("approve").resolve(),
      ).rejects.toMatchObject({ code: "capability-provider-lost" });
    });
    ...
    it("rejects stale application evidence sent by a real adapter process", async () => {
      ...
      await expect(
        ...
        message: expect.stringContaining(
          "evidence revision 1 does not match snapshot revision 2",
        ),
      });
    });
    ...
    it("fails closed when a frozen geometry guarantee degrades", async () => {
      ...
      await expect(
        terminal.getByRole("button", { name: "Approve" }).resolve(),
      ).rejects.toMatchObject({
        code: "adapter-guarantee-violation",
      });
    });
    ...
    it("focuses and fills through an application production-strategy provider without pointer support", async () => {
      ...
      if (contract === null)
        throw new Error("semantic contract was not attached");
      ...
      expect(
        (await terminal.getByTestId("name-input").semanticState())?.focused,
      ).toBe(true);
      ...
      expect(
        fillReceipt.executed.map(({ device, kind }) => `${device}:${kind}`),
      ).toEqual(["keyboard:press", "keyboard:type"]);
      await terminal.waitForText("name: [ada]");
      expect(
        await terminal.getByTestId("name-input").semanticValue(),
      ).toMatchObject({
        status: "known",
        value: "ada",
        sensitivity: "public",
    ...
    it("does not let unrelated terminal output block a focused keyboard action", async () => {
      ...
      expect(receipt.before.pairedScreenRevision).not.toBeNull();
      expect(receipt.before.pairedScreenRevision).toBeLessThan(
        receipt.before.screenRevision,
      );
      expect(receipt.before.semanticRevision).toBe(
        receipt.plan.checkpoint.semanticRevision,
      );
      expect(receipt.plan.strategy).toBe("authoritative-activate");
