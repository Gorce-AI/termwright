...
const evidence: EvidenceProvenance = Object.freeze({
  source: 'framework', method: 'instrumented', strength: 'authoritative', providerId: 'retry-fixture',
});
const supported = Object.freeze({ status: 'supported' as const, evidence });
const contract: EffectiveSessionContract = Object.freeze({
  contractId: 'retry:0', sessionId: 'retry', epoch: 0, protocol: 'termwright/2', framework: null,
...
function fixture(initial: 'disabled' | 'covered' | 'ready') {
  ...
  const node = (): SemanticNode => Object.freeze({
    ...
    geometry: Object.freeze({
      ...
      intendedRect: Object.freeze({ status: 'known', value: { row: 3, column: 8, width: 8, height: 1 }, evidence }),
      visibleRect: Object.freeze({ status: 'known', value: { row: 3, column: 8, width: 8, height: 1 }, evidence }),
    }),
  });
  const snapshot = (): SemanticSnapshot => Object.freeze({
    v: 2, sessionId: 'retry', revision: sequence, columns: 40, rows: 10,
    rootIds: Object.freeze(['save']), nodes: Object.freeze([node()]),
    coordinateSpace: Object.freeze({ status: 'known', value: 'viewport-cells', evidence }),
    hitGrid: Object.freeze({
      status: 'known', evidence,
      value: Object.freeze({ regions: Object.freeze([Object.freeze({
        recipientId: phase === 'covered' ? 'overlay' : 'save',
        rect: Object.freeze({ row: 3, column: 8, width: 8, height: 1 }),
      })]) }),
    }),
  });
  const checkpoint = (): ObservationStamp => Object.freeze({
    sessionId: 'retry', contractId: 'retry:0', epoch: 0, sequence,
  ...
  const ctx: LocatorContext = {
    ...
    semanticRevision: () => sequence, screenRevision: () => sequence, checkpoint,
    contract: () => contract, semanticNode: (id) => id === 'save' ? node() : undefined,
    hitGrid: () => snapshot().hitGrid,
    pointerRegion: (id) => id === 'save' ? Object.freeze({
      regionBounds: Object.freeze({ row: 3, column: 8, width: 8, height: 1 }),
      spans: Object.freeze([Object.freeze({ row: 3, from: 8, to: 16 })]), evidence,
    }) : undefined,
    screenRegionUnchangedSince: () => true, rows: () => Object.freeze([]),
    modes: () => Object.freeze({ mouseTracking: 'any', mouseEncoding: 'sgr', bracketedPaste: false, applicationCursorKeys: false, applicationKeypad: false, focusReporting: 'off', synchronizedOutput: false }),
    identityKind: () => 'stable', semanticBoundsAreAbsolute: () => true,
    waitForChange: async () => {
      statusWakeups += 1;
      ...
      if (statusWakeups === 3) { phase = 'ready'; sequence = 2; }
    },
    sendInput: async () => undefined, executeDeviceOperations: execute,
    beginAction: () => 'a1', endAction: () => undefined,
    errorDiagnostics: () => ({ semanticTree: true }), assertOpen: () => undefined,
  };
...
describe('Locator action retry execution', () => {
  ...
  it.each(['disabled', 'covered'] as const)(
    ...
    async (initial) => {
      ...
      expect(test.execute.mock.calls[0]?.[0].map((operation) => operation.kind)).toEqual(['down', 'up']);
