 ...
 */
export function integratedPowerShellCommand(command: readonly string[]): readonly string[] {
  if (command.length === 0) throw new TypeError('a PowerShell command needs an executable');
  let hasNoExit = false;
  for (let index = 1; index < command.length; index += 1) {
    ...
    if (value === undefined || value.startsWith('-')) {
      throw new TypeError(`PowerShell startup option ${JSON.stringify(argument)} needs a value`);
    }
    index += 1;
  }
  return [...command, ...(hasNoExit ? [] : ['-NoExit']), '-Command', powershellStartupCommand()];
}
