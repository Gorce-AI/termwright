...
import { CapacityError, TimeoutError, UnsupportedActionError } from '@termwright/driver';
...
export class ControlChannel {
  ...
  async waitForFixture(timeoutMs: number): Promise<void> {
    ...
    if (this.#everAttached) {
      ...
      throw new UnsupportedActionError('the fixture disconnected from the control channel', {
        semanticTree: false,
   ...
   * another process.
   *
   * @throws UnsupportedActionError when props cannot cross as JSON, when no
   * fixture is attached, or when the fixture refuses them
  ...
  async rerender(props: JsonProps, timeoutMs = DEFAULT_COMMAND_TIMEOUT_MS): Promise<void> {
    ...
    if (socket === null || this.#closed) {
      throw new UnsupportedActionError('the control channel has no attached fixture', {
        semanticTree: false,
    ...
    const reply = await new Promise<CommandReply>((resolve, reject) => {
      ...
      socket.write(line, (error) => {
        ...
        this.#pending = null;
        reject(
          new UnsupportedActionError(`the rerender could not be delivered: ${error.message}`, {
            semanticTree: false,
          }),
        );
      });
    ...
    if (reply.type === 'error') {
      throw new UnsupportedActionError(`the fixture refused the rerender: ${reply.detail ?? 'no detail given'}`, {
        semanticTree: false,
