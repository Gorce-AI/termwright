...
export function shouldJoin(beforeState: number, afterInfo: number): number {
  const beforeCode = (beforeState & GRAPHEME_BREAK_MASK) >> GRAPHEME_BREAK_SHIFT;
  const afterCode = (afterInfo & GRAPHEME_BREAK_MASK) >> GRAPHEME_BREAK_SHIFT;
...
export function shouldJoinBackwards(beforeInfo: number, afterState: number): number {
  const afterCode = (afterState & GRAPHEME_BREAK_MASK) >> GRAPHEME_BREAK_SHIFT;
  const beforeCode = (beforeInfo & GRAPHEME_BREAK_MASK) >> GRAPHEME_BREAK_SHIFT;
  if (_shouldJoin(beforeCode, afterCode)) {
