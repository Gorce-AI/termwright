...
describe('parseAdapterMessage', () => {
  ...
  it('flags a foreign protocol version distinctly from malformed input', () => {
    expect(adapterCode({ ...hello(), protocol: 'termwright/1' })).toBe('bad-version');
  ...
  it('rejects duplicate identities and malformed capabilities without registration-order ownership', () => {
    ...
    expect(adapterCode({ ...hello(), providers: [provider, provider] })).toBe('malformed');
    expect(
      adapterCode({
        ...hello(),
        providers: [
...
describe('parseDriverMessage', () => {
  ...
  it('still rejects a known limits key of the wrong type', () => {
    expect(driverCode({ ...helloAck(), limits: { ...LIMITS, maxNodes: 'lots' } })).toBe(
      'malformed',
    );
  });
