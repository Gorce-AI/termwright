...
describe("executable capability graph", () => {
  ...
  it("contains the wire vocabularies rather than maintaining parallel maps", () => {
    expect(
      ADAPTER_CAPABILITIES.map((id) => capabilityNode(`adapter.${id}`).id),
    ).toHaveLength(ADAPTER_CAPABILITIES.length);
    expect(
      PROBE_CAPABILITIES.map((id) => capabilityNode(`probe.${id}`).id),
    ).toHaveLength(PROBE_CAPABILITIES.length);
    expect(
      EVIDENCE_PROVIDER_CAPABILITIES.map(
        (id) => capabilityNode(`provider.${id}`).id,
      ),
    ).toHaveLength(EVIDENCE_PROVIDER_CAPABILITIES.length);
    expect(
      SESSION_CAPABILITIES.map((id) => capabilityNode(`session.${id}`).id),
    ).toHaveLength(SESSION_CAPABILITIES.length);
  });
  ...
  it("keeps diagnostic facts from producing certified session guarantees", () => {
    ...
    expect(
      CAPABILITY_GRAPH.edges.filter(
        ({ kind, from, to }) =>
          kind === "produces" &&
          diagnosticSources.has(from) &&
          to.startsWith("session."),
      ),
    ).toEqual([]);
  ...
  it("keeps paint/focus/scroll producers explicit", () => {
    expect(
      CAPABILITY_GRAPH.edges.filter(
        ({ kind, to }) =>
          kind === "produces" && to === "session.painted-region",
      ),
    ).toEqual([
      {
  ...
  it("binds planner requirements to the canonical Condition vocabulary", () => {
    for (const node of CAPABILITY_GRAPH.nodes) {
      for (const condition of node.conditions ?? [])
        expect(CONDITION_KINDS).toContain(condition);
    }
