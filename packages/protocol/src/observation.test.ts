...
describe("qualified geometry", () => {
  ...
  it("rejects ambiguous, non-canonical hit regions", () => {
    ...
    expect(
      validateSnapshot({ ...base, hitGrid }, DEFAULT_LIMITS),
  ...
  it("computes exact viewport ratios for partial, full and offscreen boxes", () => {
    expect(
      viewportIntersection({ row: 1, column: 2, width: 4, height: 2 }, 10, 5),
    ).toEqual({
    ...
    expect(
      viewportIntersection({ row: 4, column: 8, width: 4, height: 2 }, 10, 5),
    ).toEqual({
    ...
    expect(
      viewportIntersection({ row: 5, column: 10, width: 4, height: 2 }, 10, 5),
    ).toEqual({
  ...
  it("satisfies intersection symmetry and area bounds over a deterministic grid", () => {
      ...
      for (let column = -1; column <= 2; column += 1) {
        for (let width = 0; width <= 2; width += 1) {
          for (let height = 0; height <= 2; height += 1)
            rects.push({ row, column, width, height });
        }
      }
    }
    for (const a of rects) {
      for (const b of rects) {
        ...
        expect(rectArea(ab)).toBeLessThanOrEqual(
          Math.min(rectArea(a), rectArea(b)),
        );
