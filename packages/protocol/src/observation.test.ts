...
describe('qualified geometry', () => {
  ...
  it('rejects a v2 hit grid that claims an unknown recipient', () => {
    const unsupported = { status: 'unsupported', capability: 'visible-rect', reason: 'framework-unobservable' };
    const snapshot = {
      v: 2, sessionId: 's', revision: 1, columns: 10, rows: 5,
      rootIds: ['n1'],
  ...
  it('rejects ambiguous, non-canonical hit regions', () => {
    const unsupported = { status: 'unsupported', capability: 'visible-rect', reason: 'framework-unobservable' } as const;
    const base = {
      v: 2, sessionId: 's', revision: 1, columns: 10, rows: 5,
      rootIds: ['n1'],
      nodes: [{ id: 'n1', role: 'button', name: 'Approve', geometry: { displayed: { status: 'known', value: true, evidence: 'probe' }, intendedRect: unsupported, visibleRect: { ...unsupported } } }],
    ...
    expect(validateSnapshot({ ...base, hitGrid }, DEFAULT_LIMITS)).toMatchObject({ ok: false, code: 'bad-rect' });
    expect(validateSnapshot({ ...base, hitGrid: { ...hitGrid, value: { regions: [
      { rect: { row: 0, column: 1, width: 3, height: 2 }, recipientId: 'n1' },
    ] } } }, DEFAULT_LIMITS)).toMatchObject({ ok: false, code: 'bad-rect' });
  });
  ...
  it('computes exact viewport ratios for partial, full and offscreen boxes', () => {
    expect(viewportIntersection({ row: 1, column: 2, width: 4, height: 2 }, 10, 5)).toEqual({
      rect: { row: 1, column: 2, width: 4, height: 2 }, ratio: 1, fullyInside: true,
    });
    expect(viewportIntersection({ row: 4, column: 8, width: 4, height: 2 }, 10, 5)).toEqual({
      rect: { row: 4, column: 8, width: 2, height: 1 }, ratio: 0.25, fullyInside: false,
    });
    expect(viewportIntersection({ row: 5, column: 10, width: 4, height: 2 }, 10, 5)).toEqual({
      rect: { row: 5, column: 10, width: 0, height: 0 }, ratio: 0, fullyInside: false,
    });
  });
  ...
  it('defines every spatial relation without axis inversion', () => {
    ...
    expect(spatialRelation(inner, 'right-of', { row: 2, column: 0, width: 3, height: 2 })).toBe(true);
    expect(spatialRelation(inner, 'below', { row: 0, column: 3, width: 2, height: 2 })).toBe(true);
  });
});
