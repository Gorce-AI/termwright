...
export type CapabilityNodeCategory =
  (typeof CAPABILITY_NODE_CATEGORIES)[number];
...
export type EvidenceProviderCapability =
  (typeof EVIDENCE_PROVIDER_CAPABILITIES)[number];
...
export type PublicCapabilityConsumer =
  (typeof PUBLIC_CAPABILITY_CONSUMERS)[number];
...
export type CapabilityConformanceClaimId =
  (typeof CAPABILITY_CONFORMANCE_CLAIMS)[number];
...
const settle = (): CapabilityRemediation =>
  Object.freeze({
    ...
    message:
      "Wait for one paired committed observation before using this capability.",
  });
const producerNodes: readonly CapabilityGraphNode[] = [
  ...ADAPTER_CAPABILITIES.map((id) => ({
    ...
    remediation: certified(
      `Use an exact certified adapter which publishes ${id}.`,
    ),
  })),
  ...PROBE_CAPABILITIES.map((id) => ({
    ...
    remediation: certified(
      `Use exact certified instrumentation which publishes ${id}.`,
    ),
  })),
  ...
  })),
  {
    ...
    remediation: runtime(
      "writable-pty",
      "Launch the application in a writable real PTY.",
    ),
  },
  {
    ...
    layer: "producer",
    description:
      "The terminal backend authoritatively observes application mouse modes.",
    remediation: runtime(
      "terminal-input-modes-authoritative",
...
const sessionNodes: readonly CapabilityGraphNode[] = [
  ...
  sessionNode(
    ...
    "claim.stable-identity-authoritative",
    certified(
      "Use a certified retained-identity adapter or explicit SemanticKey contract.",
    ),
  ),
  sessionNode(
    "intended-geometry",
    ...
    "claim.intended-geometry-authoritative",
    certified(
      "Use an adapter instrumented at the authoritative layout/render boundary.",
    ),
  ),
  sessionNode(
    "clipped-geometry",
...
const runtimeNodes: readonly CapabilityGraphNode[] = [
  ...
  },
  {
    ...
    layer: "runtime",
    description:
      "The application currently enables compatible mouse reporting.",
    remediation: runtime(
      "mouse-reporting-enabled",
...
const strategyNodes: readonly CapabilityGraphNode[] = [
  ...
  },
  {
    ...
    layer: "strategy",
    description:
      "Activate through a framework-declared keyboard convention and real PTY input.",
    remediation: certified(
      "Expose an authoritative semantic keyboard action convention.",
    ),
  },
  {
    ...
    remediation: provider(
      "pointer-evidence",
      "Provide authoritative pointer evidence.",
    ),
  },
  {
...
const diagnosticNodes: readonly CapabilityGraphNode[] = [
  {
    ...
    remediation: certified(
      "Use certified semantic evidence for public queries.",
    ),
  },
  {
    ...
    remediation: certified(
      "Use certified geometry evidence for action planning.",
    ),
  },
  {
    ...
    remediation: certified(
      "Use a universally certified render-order producer.",
    ),
  },
  {
    id: "diagnostic.logs",
...
export function validateCapabilityGraph(): CapabilityGraphValidationResult {
  ...
  for (const node of CAPABILITY_GRAPH.nodes) {
    ...
    if (!CAPABILITY_NODE_LAYERS.includes(node.layer))
      errors.push(`invalid layer on ${node.id}`);
  ...
  for (const edge of CAPABILITY_GRAPH.edges) {
    if (!nodes.has(edge.from))
      errors.push(`edge source ${edge.from} is orphaned`);
  ...
  for (const node of CAPABILITY_GRAPH.nodes) {
    if (
      ...
      errors.push(
        `contract/public node ${node.id} has no mandatory conformance claim`,
      );
    }
  ...
  for (const claim of CAPABILITY_CONFORMANCE_CLAIMS) {
    if (!usedClaims.has(claim))
      errors.push(`orphan conformance claim ${claim}`);
  }
...
export function capabilityNode(id: CapabilityNodeId): CapabilityGraphNode {
  ...
  if (node === undefined)
    throw new RangeError(`unknown capability graph node ${id}`);
  return node;
}

/** Generated, deterministic remediation for a missing graph node. */
export function capabilityRemediation(
  id: CapabilityNodeId,
): CapabilityRemediation {
...
export function sessionCapabilitiesFromProducers(
  ...
  for (const edge of CAPABILITY_GRAPH.edges) {
    if (
      edge.kind !== "produces" ||
      !edge.to.startsWith("session.") ||
      !producers.has(edge.from)
    )
      continue;
...
export function resolveCapability(
  ...
  const resolve = (id: CapabilityNodeId): readonly CapabilityNodeId[] => {
    ...
    if (
      ...
      return [id];
    if (visiting.has(id))
      throw new Error(`capability graph dependency cycle at ${id}`);
    visiting.add(id);
