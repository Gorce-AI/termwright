...
const stamp = Object.freeze({
  sessionId: 's', contractId: 'c', epoch: 0, sequence: 1,
  screenRevision: 1, semanticRevision: 1, pairedScreenRevision: 1,
});
...
describe('artifact-safe device operations', () => {
  ...
  it('never copies an unclassified or sensitive input into a redacted plan', () => {
    ...
    const plan = recordActionPlan({
      actionId: 'a1', contractId: 'c', intent: { kind: 'fill' }, checkpoint: stamp,
      requirements: [], strategy: 'keyboard', operations: [
    ...
    expect(plan.operations).toEqual([
      { device: 'keyboard', kind: 'type', value: { status: 'withheld', reason: 'artifact-policy', sensitivity: 'sensitive' } },
      { device: 'keyboard', kind: 'paste', value: { status: 'withheld', reason: 'artifact-policy', sensitivity: 'sensitive' } },
      { device: 'keyboard', kind: 'press', value: { status: 'withheld', reason: 'artifact-policy', sensitivity: 'sensitive' } },
    ]);
  });
  ...
  it('records declared public values and requires raw policy for sensitive values', () => {
    ...
    const base = {
      actionId: 'a1', contractId: 'c', intent: { kind: 'type' as const }, checkpoint: stamp,
      requirements: [], strategy: 'keyboard', operations: [
  ...
  it('can only make an already-recorded receipt stricter at a later boundary', () => {
    ...
    const executable = {
      actionId: 'a1', contractId: 'c', intent: { kind: 'type' as const }, checkpoint: stamp,
      requirements: [], strategy: 'keyboard', operations: [{ device: 'keyboard' as const, kind: 'type' as const, value: sensitive(secret) }],
    };
    const rawPlan = recordActionPlan(executable, 'raw');
    const rawReceipt = { intent: executable.intent, plan: rawPlan, before: stamp, after: stamp, executed: rawPlan.operations, outcome: 'completed' as const };
