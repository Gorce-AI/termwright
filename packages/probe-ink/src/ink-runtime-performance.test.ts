  ...
  it("reports traversal, serialization, CPU, allocation and memory without a timing gate", async () => {
      ...
      createElement(
        ...
        ...Array.from({ length: hostCount }, (_, index) =>
          createElement(
            Box,
            { key: index },
            createElement(Text, null, `node-${index}`),
          ),
        ),
      ),
    ...
    try {
      ...
      const measurement = {
        ...
        mallocedMemoryDeltaBytes:
          heapAfter.malloced_memory - heapBefore.malloced_memory,
      ...
      process.stderr.write(
        `${JSON.stringify({ inkRuntimeObserverMeasurement: measurement })}\n`,
      );
    } finally {
      instance.unmount();
