...
type Hook = {
  ...
  onCommitFiberRoot(rendererId: unknown, root: { containerInfo?: unknown }): void;
...
describe('ReactCommitBridge', () => {
  ...
  it('isolates multiple Ink renderer registrations without assuming renderer id 1', () => {
    ...
    expect(
      events.map((event) => event.type === 'commit' && [event.renderer.rendererId, event.root]),
    ).toEqual([
      [73, second],
      ['ink-a', first],
    ]);
  ...
  it('reuses one installation but activates the same reconciler for each hook target', () => {
    ...
    const reconciler = {
      injectIntoDevTools: vi.fn(() => {
        const hook = (active as unknown as { __REACT_DEVTOOLS_GLOBAL_HOOK__: Hook })
          .__REACT_DEVTOOLS_GLOBAL_HOOK__;
  ...
  it('preserves inherited properties and observes commits even when a delegated callback throws', () => {
    ...
    expect(() => hook.onCommitFiberRoot(id, { containerInfo: committed })).toThrow(
      'existing hook failure',
    );
    expect(hook.rendererInterfaces).toBe(prototype.rendererInterfaces);
    expect(events).toEqual([expect.objectContaining({ type: 'commit', root: committed })]);
  });
  ...
  it('bounds the experimental Fiber walk instead of allowing unbounded commit overhead', () => {
    ...
    expect(() =>
      correlateInkHostProps({ current: { child: chain[0] as never } }, { maxFibers: 4 }),
  ...
  it('activates the reconciler without DEV=true and proves registration by injection', () => {
    ...
    try {
      expect(activateInkRendererObservation(reconciler, scope).hasInkRenderer()).toBe(true);
  ...
  it('fails closed when the reconciler does not register an Ink renderer', () => {
    expect(() =>
      activateInkRendererObservation({ injectIntoDevTools: () => false }, target()),
