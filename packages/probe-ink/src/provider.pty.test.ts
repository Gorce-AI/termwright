  ...
  () => {
    ...
    it("routes a high-level click through normal PTY mouse bytes", async () => {
      ...
      try {
        ...
        expect(
          terminal.contract()?.capabilities["pointer-hit-testing"],
        ).toMatchObject({
          status: "supported",
    ...
    it("uses production parser mode evidence when the terminal transport hides DEC modes", async () => {
      ...
      try {
        ...
        await terminal.waitForText("last: reject@");
        expect(
          terminal.contract()?.capabilities["pointer-input"],
        ).toMatchObject({
          status: "supported",
    ...
    it("delivers provider-proven focus reports through the real PTY parser path", async () => {
      ...
      try {
        await terminal.settled();
        expect(terminal.terminalState.snapshot().modes.focusReporting).toBe(
          "on",
        );
    ...
    it("fails closed when production parser modes disagree with observable VT modes", async () => {
      ...
      try {
        await expect(terminal.settled()).rejects.toBeInstanceOf(
          EvidenceConflictError,
        );
      } finally {
        await terminal.close();
    ...
    it("uses an explicit authoritative production-region contract without a negotiated hit test", async () => {
      ...
      try {
        ...
        await terminal.waitForText("last: reject@");
        expect(
          terminal.contract()?.capabilities["pointer-hit-testing"].status,
        ).toBe("unsupported");
        expect(
          receipt.plan?.requirements.find(
            ({ condition }) => condition.kind === "receives-pointer",
          ),
        ).toMatchObject({
          verdict: "satisfied",
    ...
    it("uses fresh provider evidence after a semantic target moves", async () => {
      ...
      try {
        ...
        const receipt = await terminal
          ...
          .click();
        await terminal.waitForText("last: reject@");
        expect(receipt.plan?.checkpoint.semanticRevision).toBe(
          receipt.before.semanticRevision,
        );
    ...
    it("chooses a reachable cell from a partially clipped production region", async () => {
      ...
      try {
        ...
        expect(
          (receipt.executed[0] as { column: number }).column,
    ...
    it("routes stepped semantic drag through the production PTY input path", async () => {
      ...
      try {
        ...
        expect(
          receipt.executed.filter((step) => step.kind === "move"),
        ).toHaveLength(6);
      } finally {
        await terminal.close();
      }
