...
describe.skipIf(!available)('the termwright ui worker/session bridge', () => {
  test('streams a real PTY session while the test is running', { timeout: 30_000 }, async ({ terminal }) => {
    ...
    expect(messages.some((message) => message.type === 'app-log' && message.message === 'a single record')).toBe(true);
