...
const broker: ResourceBrokerClient = Object.freeze({
  ...
  acquire: async () => { throw new Error('unit attempt did not provision terminal resources'); },
  snapshot: async () => { throw new Error('unit attempt did not request a broker snapshot'); },
  close: async () => undefined,
});

/** Explicit unit-only authority; production AttemptContext never has a fallback broker. */
export function unitAttemptOptions() {
  return Object.freeze({ budget: new TestBudget(5_000), broker, resourceProfile: Object.freeze({}) });
}
