...
describe('exact Vitest adapter seams', () => {
  it('anchors real paths to the declared host root without rewriting substring filters', () => {
    ...
    expect(
      hostRelativeFilters(['packages/termwright-cli/src/test-host.ts', 'name fragment', ''], cwd),
    ).toEqual([resolve(cwd, 'packages/termwright-cli/src/test-host.ts'), 'name fragment', '']);
  });
  ...
  it('removes only implicit default reporters and preserves explicit reporter order', () => {
    ...
    removeEmbeddedDefaultReporter({ version: '4.1.11', reporters } as Pick<Vitest, 'version'> & {
      reporters: Reporter[];
    });
  ...
  it('fails closed if the exact-certified async-leak state surface changes', () => {
    expect(
      () =>
        new ExactVitestEngine({
          version: CERTIFIED_VITEST_VERSION,
          reporters: [],
          state: { getUnhandledErrors: () => [] },
        } as unknown as Vitest),
    ).toThrow(/async-leak state surface changed/u);
  });

  it('treats profile file parallelism as permission and preserves stricter project config', () => {
    expect(
      vitestSchedulerOverrides({ pool: 'forks', maxWorkers: 4, fileParallelism: true }),
    ).toEqual({
      pool: 'forks',
      maxWorkers: 4,
    });
    expect(
      vitestSchedulerOverrides({ pool: 'forks', maxWorkers: 2, fileParallelism: false }),
    ).toEqual({
      pool: 'forks',
      maxWorkers: 2,
  ...
  it('distinguishes the default repository catalogue from explicit partial collection', () => {
    ...
    expect(vitestCatalogueScope([], { run: true, related: ['packages/ui/src/index.ts'] })).toBe(
      'targeted',
    );
  ...
  it('fails closed when the current Vitest result is missing or duplicates selected tasks', () => {
    expect(() =>
      classifyVitestResult(result([test('passed', 0, 'previous')]), new Set(['current'])),
  ...
  it('adapts native module ids and structured console output without a host', async () => {
    ...
    reporters
      .at(-1)
      ?.onUserConsoleLog?.({ content: 'structured', type: 'stdout', time: 1, size: 10 });
  ...
  it('preserves both the operation and worker-pool teardown errors', async () => {
    ...
    const failure = await new ExactVitestEngine(vitest)
      .collect([])
  ...
  it('promotes Vitest async-leak evidence into a non-certifying engine result', async () => {
    ...
    const resultWithLeak = await new ExactVitestEngine(vitest).run(
      new Set(['/workspace/leaky.test.ts']),
    );
    ...
    expect(classifyVitestResult(resultWithLeak, new Set(['selected']))).toBe(
