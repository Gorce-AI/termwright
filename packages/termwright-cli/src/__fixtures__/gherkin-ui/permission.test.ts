...
const semanticApp = join(dirname(fileURLToPath(import.meta.url)), '..', '..', '..', '..', 'driver', 'test-fixtures', 'semantic-app.mjs');
...
test('TypeScript opens the permission terminal', async ({ terminal, expect }) => {
  ...
  const app = await terminal.launch({ command: [process.execPath, semanticApp], columns: 60, rows: 10, trace: 'on' });
