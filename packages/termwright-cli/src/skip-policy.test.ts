...
describe('repository native skip policy', () => {
  it('loads only applicable exact rules and registered platform deviations', async () => {
    ...
    await writeFile(join(root, 'quality', 'applicability-skips.json'), JSON.stringify({
      version: 1,
      rules: [
        { id: 'all', file: 'all.test.ts', suite: 'adapter conformance: exact', fullName: 'always optional' },
        { id: 'windows', file: 'win.test.ts', fullName: 'win only', platforms: ['win32'], required: true },
      ],
    }));
    await writeFile(join(root, 'quality', 'platform-deviations.json'), JSON.stringify({
      version: 1,
      deviations: [{
        id: 'POSIX-DEVIATION',
        predicate: 'non-win32',
    ...
    await expect(loadRepositorySkipDeclarations(root, 'linux')).resolves.toEqual([
      { id: 'all', file: 'all.test.ts', suite: 'adapter conformance: exact', fullName: 'always optional', required: false },
      {
  ...
  it('fails closed on malformed policy instead of dropping it', async () => {
    ...
    await mkdir(join(root, 'quality'));
    await writeFile(join(root, 'quality', 'applicability-skips.json'), '{"version":1,"rules":[{}]}');
  ...
  it('rejects an unknown platform instead of silently making a rule inapplicable', async () => {
    ...
    await expect(loadRepositorySkipDeclarations(root, 'linux')).rejects.toThrow(/invalid platforms/u);
  });
});
