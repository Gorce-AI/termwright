...
const forbidden = [
  { label: 'a deleted legacy UI path', pattern: /(?:from\s*|import\s*)['"][^'"]*(?:\/old\/|\.\.\/old(?:\/|['"]))/u },
  { label: 'Lit rendering', pattern: /(?:from\s*|import\s*)['"]lit-html(?:\/[^'"]*)?['"]/u },
  { label: 'the legacy stylesheet', pattern: /(?:from\s*|@import\s*)['"][^'"]*old\/styles\.css['"]/u },
];
...
if (failures.length > 0) {
  process.stderr.write(`Fresh UI boundary violated:\n${failures.map((failure) => `- ${failure}`).join('\n')}\n`);
  process.exitCode = 1;
} else {
