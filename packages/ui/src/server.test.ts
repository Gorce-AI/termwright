...
import { startUiServer, type UiServer, type UiServerOptions } from './server.js';
import {
  createRunId,
  ...
  const server = await startUiServer({
    ...
    ...(discovery === undefined
      ...
      : {
          discovery: {
            cwd: discovery.cwd,
            ...(discovery.watch === undefined ? {} : { watch: discovery.watch }),
...
describe('live mode', () => {
  ...
  it('rejects lifecycle events from a session producer', async () => {
    ...
    await expect(producer.closed()).resolves.toMatchObject({
      code: 1008,
      reason: 'producer cannot publish run-start',
    });
  ...
  it('rejects duplicate or regressing semantic revisions from an owned session', async () => {
    ...
    await waitUntil(
      () => server.hub.backlog.some((message) => message.type === 'semantic'),
  ...
  it('expires producer ownership at the next authoritative run generation', async () => {
    ...
    await waitUntil(
      () => server.hub.backlog.some((message) => message.type === 'session'),
      'old session claim',
    );
  ...
  it('disconnects a producer after a malformed frame', async () => {
    ...
    await expect(producer.closed()).resolves.toMatchObject({
      code: 1008,
      reason: 'malformed producer frame',
    });
    expect(
  ...
  it('publishes the project\u2019s tests before anything runs', async () => {
    ...
    expect(
      discovered?.type === 'tests-discovered' && discovered.tests.map((test) => test.title),
  ...
  it('reports collection failure without fabricating an empty catalogue', async () => {
    ...
    const viewer = await Viewer.connect(server);
    await viewer.until(
      (messages) => messages.some((message) => message.type === 'collection-failed'),
      'the collection failure',
    );
    expect(
      viewer.received.findLast((message) => message.type === 'collection-failed'),
    ).toMatchObject({
  ...
  it('re-lists .feature changes and publishes an empty catalogue after the last case is deleted', async () => {
    ...
    expect(
      viewer.received.findLast((message) => message.type === 'tests-discovered'),
    ).toMatchObject({
      tests: [],
    });
    viewer.close();
  ...
  it('does not let a slower stale discovery overwrite a newer feature listing', async () => {
    ...
    await viewer.until(
      (messages) =>
        messages.some(
          (message) => message.type === 'tests-discovered' && message.tests[0]?.title === 'new',
        ),
      'the newer listing',
    );
    finishFirst?.(stale);
    ...
    expect(
      server.hub.backlog.some(
        (message) => message.type === 'tests-discovered' && message.tests[0]?.title === 'stale',
      ),
    ).toBe(false);
    viewer.close();
  });
...
describe('post-mortem mode', () => {
  it('replays the archive timeline on connect and serves state at a moment', async () => {
    ...
    expect(Buffer.from(state.castPrefixB64, 'base64').toString('utf8')).toContain(
      'Permission required',
    );
...
describe('starting a run from the panel', () => {
  ...
  it('accepts admission immediately and reports later infrastructure failure on the event stream', async () => {
    ...
    expect(response.status).toBe(202);
    await waitUntil(
      () => server.hub.backlog.some((message) => message.type === 'run-infrastructure-failed'),
      'the authoritative infrastructure failure',
    );
    expect(
      server.hub.backlog.findLast((message) => message.type === 'run-infrastructure-failed'),
    ).toMatchObject({
  ...
  it('atomically refuses a second tab and permits a later run after the first finishes', async () => {
    ...
    await legacy.until(
      (messages) =>
        messages.some(
          (message) =>
            message.type === 'control-result' && message.requestId === 'legacy-frame-barrier',
        ),
  ...
  it('accepts only exact native RunnerTaskIds from the scoped provider catalogue', async () => {
    ...
    expect(asked).toEqual([['runner-task:00000000-0000-4000-8000-000000000001']]);
  });
  ...
  it('refuses browser runs while the initial watcher run is live', async () => {
    ...
    expect((await api(server, '/api/run', { method: 'POST', body: '{}' })).status).toBe(202);
  ...
  it('does not let a producer forge a terminal edge for an authoritative run', async () => {
    ...
    await expect(producer.closed()).resolves.toMatchObject({
      code: 1008,
      reason: 'producer cannot publish run-end',
    });
    expect((await api(server, '/api/run', { method: 'POST', body: '{}' })).status).toBe(409);
    expect(asked).toEqual([]);
  ...
  it('holds input back while pick mode is on', async () => {
    ...
    await viewer.until(
      (messages) =>
        messages.some(
          (message) =>
            message.type === 'control-result' && message.requestId === 'pick-on' && message.ok,
        ),
