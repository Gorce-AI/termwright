...
describe('ariaElementFor', () => {
  ...
  it('marks a modal dialog as modal', () => {
    expect(
      ariaElementFor(node({ id: 'n1', role: 'dialog', name: 'Permission', state: { modal: true } })).attrs[
        'aria-modal'
      ],
    ).toBe('true');
    // Only a dialog can be modal; the flag on anything else is dropped.
    expect(
      ariaElementFor(node({ id: 'n1', role: 'region', state: { modal: true } })).attrs['aria-modal'],
    ).toBeUndefined();
  });

  it('emits aria-checked only where checking means something', () => {
    expect(ariaElementFor(node({ id: 'n1', role: 'checkbox', state: { checked: 'mixed' } })).attrs).toEqual({
      'aria-checked': 'mixed',
    });
    expect(ariaElementFor(node({ id: 'n1', role: 'radio', state: { checked: true } })).attrs).toEqual({
      'aria-checked': 'true',
    });
    expect(ariaElementFor(node({ id: 'n1', role: 'button', state: { checked: true } })).attrs).toEqual({});
  });

  it('falls back from aria-selected to aria-current where ARIA would ignore it', () => {
    expect(ariaElementFor(node({ id: 'n1', role: 'tab', state: { selected: true } })).attrs).toEqual({
      'aria-selected': 'true',
    });
    expect(ariaElementFor(node({ id: 'n1', role: 'listitem', state: { selected: true } })).attrs).toEqual({
      'aria-current': 'true',
    });
    // Not selected and not selectable: nothing to say.
    expect(ariaElementFor(node({ id: 'n1', role: 'listitem', state: { selected: false } })).attrs).toEqual({});
  });

  it('gives a heading its level', () => {
    expect(ariaElementFor(node({ id: 'n1', role: 'heading', state: { level: 2 } })).attrs['aria-level']).toBe('2');
    expect(ariaElementFor(node({ id: 'n1', role: 'button', state: { level: 2 } })).attrs['aria-level']).toBeUndefined();
  });

  it('describes position in a set', () => {
    expect(
      ariaElementFor(node({ id: 'n1', role: 'listitem', state: { positionInSet: 2, setSize: 5 } })).attrs,
  ...
  it('reads a progressbar’s value as a number when it is one', () => {
    expect(ariaElementFor(node({ id: 'n1', role: 'progressbar', value: '42' })).attrs['aria-valuenow']).toBe('42');
    expect(ariaElementFor(node({ id: 'n1', role: 'progressbar', value: 'almost' })).attrs['aria-valuetext']).toBe(
      'almost',
    );
  });
  ...
  it('passes a description through', () => {
    expect(
      ariaElementFor(node({ id: 'n1', role: 'button', name: 'Save', description: 'writes the file' })).attrs[
        'aria-description'
      ],
...
describe('ariaTextFor', () => {
  it('renders text and textbox content, since that is their meaning', () => {
    expect(ariaTextFor(node({ id: 'n1', role: 'text', value: 'running: ls -la' }))).toBe('running: ls -la');
