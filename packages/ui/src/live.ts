...
export async function inspectNodeActionability(
  ...
  if (snapshot === null || snapshot === undefined)
    throw new Error('the live session has no committed semantic tree');
  if (!snapshot.nodes.some((node) => node.id === nodeId))
    throw new Error(`semantic node ${nodeId} is not attached at revision ${snapshot.revision}`);
  if (source.locatorForRef === undefined)
    throw new Error('the live session does not expose production Locator actionability');
  const locator = source.locatorForRef(`semantic:${nodeId}@${snapshot.revision}`);
  const explanations = await Promise.all(
    INSPECTED_ACTIONS.map((action) => locator.actionability(action)),
  );
  ...
  if (
    ...
    explanations.some(
      (entry) =>
        entry.checkpoint.contractId !== checkpoint.contractId ||
        entry.checkpoint.sequence !== checkpoint.sequence,
    )
  )
    ...
    switch (recorded.type) {
      ...
      case 'semantic-revision': {
        ...
        sink.publish({
          ...
          sessionId,
          revision,
          snapshot: projectSemanticSnapshotForArtifact(snapshot),
        });
        break;
      }
      ...
      case 'action': {
        ...
        sink.publish({
            ...
            : {
                actionPlan: {
                  ...
                  requirements: event.receipt.plan.requirements.map((requirement) => ({
                    kind: requirement.condition.kind,
                    ...('target' in requirement.condition
                      ? { target: requirement.condition.target }
                      : {}),
                    verdict: requirement.verdict,
                    observation: requirement.observation.status,
                    ...('evidence' in requirement.observation
          ...
          ...(event.actionability === undefined
            ? {}
            : { actionability: toUiActionability(event.actionability) }),
          ...(stepId === undefined ? {} : { stepId }),
        });
        break;
      }
