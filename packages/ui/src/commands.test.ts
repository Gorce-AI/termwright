    ...
    const rows = buildCommandLog([
      {
        ...
        receipt: {
          ...
          plan: {
            ...
            requirements: [
              {
                condition: { kind: 'receives-pointer', target: 'b1@7' },
                checkpoint: stamp,
            ...
            physicalRegion: {
              ...
              intendedRect: { row: 1, column: 1, width: 2, height: 1 },
    ...
    const rows = buildCommandLog([
      {
        ...
        receipt: {
          intent: { kind: 'click' },
          plan: { actionId: 'a1', contractId: 's:0', strategy: 'pointer', requirements: 'forged' },
          before: { sequence: 1 },
          after: { sequence: 2 },
          executed: [],
        },
    ...
    const rows = buildCommandLog([
      {
        ...
        actionability: {
          actionable: false,
          intent: { kind: 'click', targetRef: 'semantic:save@7' },
          checkpoint: stamp,
          requirements: [
            {
              condition: { kind: 'receives-pointer', target: 'save@7' },
              checkpoint: stamp,
    ...
    const rows = buildCommandLog([
      {
        ...
        actionability: {
          actionable: false,
          intent: { kind: 'click' },
          checkpoint: { contractId: 's:0', sequence: 1 },
          requirements: 'forged',
        },
      },
    ]);
