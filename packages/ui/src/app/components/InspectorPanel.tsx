...
export function InspectorPanel({
  ...
  readonly onInspectActionability?: (
    sessionId: string,
    nodeId: string,
  ...
  useEffect(() => {
    ...
    void onInspectActionability(session.sessionId, selectedNode.id)
      .then((results) => {
  ...
  return (
    <section className="tw-inspector" aria-label="Execution inspector">
      <header
        className="tw-inspector-tabs"
        role="tablist"
        aria-label="Inspector views"
        onKeyDown={(event) => moveTab(event, tab, setTab)}
      >
        <InspectorTabButton tab="tree" current={tab} onSelect={setTab} icon={Waypoints}>
          Tree
        </InspectorTabButton>
        <InspectorTabButton tab="semantic" current={tab} onSelect={setTab} icon={Braces}>
          Semantic
        </InspectorTabButton>
        <InspectorTabButton tab="logs" current={tab} onSelect={setTab} icon={FileText}>
          Logs
        </InspectorTabButton>
        <span className="tw-revision">
          {session?.revision === null || session === null
            ? 'no revision'
...
function ContractSummary({ contract }: { readonly contract: EffectiveSessionContract | null }) {
  if (contract === null) {
    return (
      <section className="tw-contract-summary">
        <h3>Effective session contract</h3>
        <p>Negotiation has not produced a semantic contract.</p>
      </section>
    );
  }
  const supported = Object.entries(contract.capabilities).filter(
    ([, value]) => value.status === 'supported',
  );
  const unavailable = Object.entries(contract.capabilities).filter(
    ([, value]) => value.status === 'unsupported',
  );
  const supports = (id: keyof EffectiveSessionContract['capabilities']) =>
    contract.capabilities[id].status === 'supported';
  const effective = [
    [
      'click',
      supports('pointer-input') && supports('pointer-geometry') && supports('pointer-hit-testing'),
    ],
    [
      'hover',
      supports('pointer-input') && supports('pointer-geometry') && supports('pointer-hit-testing'),
  ...
  return (
    <section className="tw-contract-summary" aria-label="Effective session contract">
      <header>
        <div>
        ...
        <CopyButton label="Copy contract id" value={contract.contractId} />
      </header>
      <p>
        {contract.framework === null
          ? 'No framework adapter'
          : `Certified adapter ${contract.framework.certificationId}`}
      </p>
      <dl className="tw-property-grid">
        <Property label="epoch" value={String(contract.epoch)} />
        <Property
          label="terminal"
          value={`${contract.terminal.profile} · ${contract.terminal.platform}`}
        />
        <Property
          label="mouse modes"
          value={contract.terminal.mouseModesObservable ? 'observable' : 'unobservable'}
        />
        <Property
          label="pointer input"
          value={
            supports('pointer-input')
              ? 'supported; current mouse mode is a runtime precondition'
              : 'unsupported'
          }
        />
      </dl>
      <h4>Effective API contract</h4>
      <ul className="tw-effective-api">
        {effective.map(([action, available]) => (
          <li key={action} data-available={available}>
            <span>{available ? '✓' : '–'}</span>
            <strong>{action}</strong>
            <small>{available ? 'contract supported' : 'required capability unavailable'}</small>
          </li>
        ))}
      </ul>
      <h4>Available API evidence</h4>
      <ul className="tw-contract-capabilities">
        {supported.map(([id, value]) =>
          value.status === 'supported' ? (
            <li key={id}>
              <strong>{id}</strong>
              <span>
                {value.evidence.source}/{value.evidence.method} · {value.evidence.strength}
              </span>
              <small>{value.evidence.providerId}</small>
            </li>
          ) : null,
        )}
      </ul>
      {contract.providers.filter((provider) => provider.kind === 'application').length ===
      0 ? null : (
        <>
          <h4>Application providers</h4>
          <ul className="tw-contract-capabilities">
            {contract.providers
              .filter((provider) => provider.kind === 'application')
              .map((provider) => (
                <li key={provider.id}>
                  <strong>
                  ...
                  <span>
                    {provider.kind === 'application' ? provider.capabilities.join(', ') : ''}
                  </span>
                </li>
              ))}
          </ul>
      ...
      {unavailable.length === 0 ? null : (
        <details>
          <summary>{unavailable.length} unsupported capabilities</summary>
          <ul>
    ...
    <li role="none">
      <div className="tw-semantic-node-row">
        <button
          ...
          onPointerEnter={() => onPreviewNode?.(node, snapshot)}
          onPointerLeave={() => onPreviewNode?.(null, null)}
          onFocus={() => {
            roving.onFocus();
            onPreviewNode?.(node, snapshot);
          }}
          onBlur={() => onPreviewNode?.(null, null)}
          onClick={() => {
            onSelect(node.id);
      ...
      {descendants.length === 0 || !open ? null : (
...
function SemanticDetail({
  ...
  const intendedRect =
    node.geometry.intendedRect.status === 'known' ? node.geometry.intendedRect.value : undefined;
  ...
  return (
    <div className="tw-semantic-detail">
      <header>
        <div>
          <span>{node.role}</span>
          <h3>{node.name || 'Unnamed node'}</h3>
        </div>
        <CopyButton label="Copy node ref" value={node.id} />
      </header>
      <div className="tw-state-chips">
        {states.length === 0 ? (
          <em>no portable state</em>
        ) : (
          states.map(([key, value]) => (
            <span key={key} data-on={value === true}>
              {key}: {String(value)}
            </span>
          ))
        )}
        ...
        <Property
          ...
          value={
            visibleRect === undefined
              ...
              : `${visibleRect.column},${visibleRect.row} · ${visibleRect.width}×${visibleRect.height}`
          }
        />
        <Property
          label="intended region"
          value={
            intendedRect === undefined
              ? node.geometry.intendedRect.status
              : `${intendedRect.column},${intendedRect.row} · ${intendedRect.width}×${intendedRect.height}`
          }
        />
        <Property
          label="application scroll"
          value={
            scroll === undefined
              ? node.scroll?.status
              : `${scroll.axis} · ${scroll.offset}+${scroll.viewport}/${scroll.extent}`
          }
        />
        <Property
          label="painted region"
          value={
            painted === undefined
              ? node.paintedRegion?.status
              : `${painted.regionBounds.column},${painted.regionBounds.row} · ${painted.regionBounds.width}×${painted.regionBounds.height} · ${painted.spans.length} spans`
          }
        />
      ...
      <section className="tw-semantic-actions">
        <h4>Capabilities</h4>
        <div>
          {node.actions?.map((action) => <span key={action}>{action}</span>) ?? (
            <em>none declared</em>
          )}
        </div>
      </section>
      ...
      {recorder === undefined ? null : (
        <section className="tw-semantic-actions">
          <h4>Recorder</h4>
          <NodeRecorderActions node={node} recorder={recorder} labelled />
        </section>
      )}
      {node.extended === undefined ? null : (
        <details className="tw-semantic-extended">
          <summary>Extended application state</summary>
          <pre>{JSON.stringify(node.extended, null, 2)}</pre>
        </details>
      )}
      <details className="tw-semantic-extended">
        <summary>View raw node</summary>
        <div className="tw-raw-toolbar">
          <CopyButton label="Copy JSON" value={JSON.stringify(node, null, 2)} />
        </div>
        <pre>{JSON.stringify(node, null, 2)}</pre>
      </details>
      {snapshot === null ? null : (
        <small className="tw-semantic-provenance">
          revision {snapshot.revision} · {snapshot.columns}×{snapshot.rows} · session{' '}
          {snapshot.sessionId}
        </small>
      )}
    </div>
...
function ActionabilityInspector({
  state,
}: {
  readonly state: {
    readonly loading: boolean;
  ...
  return (
    <section className="tw-live-actionability" aria-label="Live actionability">
      <h4>Live actionability</h4>
      {state.loading ? (
        <p>Planning against the committed observation…</p>
      ) : state.error !== undefined ? (
        <p role="status">Unavailable: {state.error}</p>
      ) : state.results === undefined ? (
        <p>Select a live semantic node to inspect actions.</p>
      ) : (
        <ul>
          {state.results.map((result) => (
            <li key={result.kind} data-actionable={result.actionable}>
              <header>
                <span>{result.actionable ? '✓' : '✗'}</span>
                <strong>Can {result.kind}?</strong>
                <small>revision {result.sequence}</small>
              </header>
              <p>
                {result.actionable
                  ? `Yes · ${result.strategy ?? 'production strategy'}`
                  : result.reason === undefined
                    ? 'No · requirements are inconclusive'
                    : `No · ${result.reason.code}: ${result.reason.message}`}
              </p>
              <details>
                <summary>Why?</summary>
                <ul>
                  {result.requirements.map((requirement, index) => (
                    <li key={`${requirement.kind}:${index}`} data-verdict={requirement.verdict}>
                      <span>
...
function NodeRecorderActions({
  ...
  const click = (
    <button
      type="button"
      tabIndex={tabIndex}
      aria-label={`Record click on ${node.id}`}
      onClick={() => recorder.onClickNode(node.id)}
    >
  ...
  const visible = (
    <button
      type="button"
      tabIndex={tabIndex}
      aria-label={`Assert ${node.id} is visible`}
      onClick={() => recorder.onAssertNode(node.id)}
    >
      <ShieldCheck aria-hidden="true" size={13} />
      {labelled ? 'Visible' : null}
    </button>
  );
  return (
    <span className="tw-record-node-actions">
      {labelled ? (
        click
      ) : (
        <Tooltip label={`Record click on ${node.name || node.id}`}>{click}</Tooltip>
      )}
      {labelled ? (
        visible
      ) : (
        <Tooltip label={`Assert ${node.name || node.id} is visible`}>{visible}</Tooltip>
      )}
    </span>
  );
}
...
function InspectorTabButton({
  ...
  return (
    <button
      id={`tw-inspector-tab-${tab}`}
      type="button"
      role="tab"
      aria-selected={tab === current}
      aria-controls={`tw-inspector-panel-${tab}`}
      tabIndex={tab === current ? 0 : -1}
      className="tw-inspector-tab"
      onClick={() => onSelect(tab)}
    >
      <Icon aria-hidden="true" size={14} /> {children}
    </button>
  );
}
...
function InspectorEmpty({
  ...
  return (
    <div className="tw-inspector-empty">
