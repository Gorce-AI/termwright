...
export function AppShell({ project, route, connected, features, onRoute, children }: AppShellProps) {
  const { preferences, updatePreferences } = usePreferences();
  const navigationExpanded = preferences.navigationExpanded;
  const availableNavigation = navigation.filter(({ route: destination }) => (
    destination !== 'specs' || features.live
  ) && (
    destination !== 'runs' || features.history
  ));
  return (
      ...
      <aside className="tw-nav" aria-label="Primary navigation">
        ...
        <nav className="tw-nav-links">
          {availableNavigation.map(({ route: destination, label, icon: Icon }) => (
            <Tooltip key={destination} label={label} placement="right"><button
              type="button"
        ...
        <Tooltip label={navigationExpanded ? 'Collapse navigation' : 'Expand navigation'} placement="right"><button
          type="button"
        ...
        >
          {navigationExpanded ? <PanelLeftClose aria-hidden="true" size={17} /> : <PanelLeftOpen aria-hidden="true" size={17} />}
          <span>{navigationExpanded ? 'Collapse' : 'Expand'}</span>
        </button></Tooltip>

        <div className="tw-nav-foot" title={[project?.name, project?.branch].filter(Boolean).join(' · ')}>
          <span className="tw-connection-dot" data-connected={features.live ? connected : true} aria-hidden="true" />
          <span>{features.live ? (connected ? 'Runner connected' : 'Waiting for runner') : 'Recording loaded'}</span>
        </div>
      </aside>

      <main className="tw-main">
        <h1 className="sr-only">{route === 'runner' ? 'Test run' : route[0]?.toUpperCase() + route.slice(1)}</h1>
        {children}
      </main>
    </div>
  );
