import { useCallback, useEffect, useMemo, useRef, type KeyboardEvent, type RefCallback } from 'react';
...
export function useTreeNavigation(options: RovingTreeOptions) {
  ...
  const activeId = options.selectedId !== null && visibleIds.has(options.selectedId)
    ? options.selectedId
  ...
  const onKeyDown = useCallback((event: KeyboardEvent<HTMLElement>) => {
    ...
    const next = navigateTree(options.rows, {
      selectedId: activeId,
