...
describe('application evidence provider lifecycle', () => {
  ...
  it('rejects duplicate ids and late registration without registration-order ownership', () => {
    ...
    expect(() => registerPointerEvidenceProvider(provider())).toThrow(/already registered/);
  ...
  it('publishes the exact production focus recipient, including authoritative absence', () => {
    ...
    expect(
      ...
      { status: 'available', focusState: { status: 'focused', recipientId: 'resolved:editor' } },
    ]);
    expect(
      lease.collect({
        sessionId: 's',
  ...
  it('publishes authoritative production terminal parser modes', () => {
    ...
    const lease = freezeEvidenceProviders();
    expect(lease.registrations[0]?.capabilities).toEqual(['terminal-input-modes']);
    expect(
      lease.collect({
        sessionId: 's',
        revision: 2,
  ...
  it.each([
    [
      ...
      (column: number, row: number) =>
        row === 2 && column >= 4 && column < 9 ? { semanticId: 'reject' } : null,
    ],
    [
    ...
    [
      'the wrong recipient',
      (column: number, row: number) =>
        row === 2 && column >= 4 && column < 10 ? { semanticId: 'approve' } : null,
    ],
  ...
  it('isolates explicit registries and independent parallel session leases', () => {
    ...
    firstLease.close();
    expect(() => first.registerPointer({ ...late, id: 'still-late' })).toThrow(/frozen/);
    parallelLease.close();
  ...
  it('fails closed when a provider publishes evidence outside its frozen declaration', () => {
    ...
    expect(
      registry.freeze().collect({ sessionId: 's', revision: 1, columns: 20, rows: 5 }),
    ).toMatchObject([
      {
        status: 'violation',
