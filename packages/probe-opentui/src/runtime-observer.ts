...
type RuntimeCommand =
  ...
  | {
      readonly action: 'pushScissorRect';
      readonly x: number;
      readonly y: number;
...
export function installRuntimeObserver(
  ...
  const rootRenderShadow = () =>
    shadowMethod(
      renderer.root,
      'render',
      (originalRootRender) =>
        function (buffer: RuntimeBuffer, deltaTime: number) {
          if (disposed || fatal !== undefined)
          ...
          try {
            ...
            violate(
              `OpenTUI runtime observation cannot begin: ${error instanceof Error ? error.message : String(error)}`,
            );
            return Reflect.apply(originalRootRender, this, [buffer, deltaTime]);
          }
          ...
          const wrapRenderable = (renderable: RuntimeRenderable): void => {
            ...
            try {
                ...
                (original) =>
                  function (...args) {
                    ...
                    if (
                      ...
                      fail(
                        `OpenTUI renderable ${String(renderable.num)} returned an asynchronous render result`,
                      );
                    } else {
                      record(renderable);
                    }
              ...
              fail(
                `cannot observe OpenTUI render command ${String(renderable.num)}: ${error instanceof Error ? error.message : String(error)}`,
              );
            }
          };
          const inspectCommand = (candidate: unknown): void => {
            ...
            if (command.action === 'render') {
              if (
                command.renderable === undefined ||
                typeof command.renderable.render !== 'function'
              ) {
            ...
            if (
              !['pushScissorRect', 'popScissorRect', 'pushOpacity', 'popOpacity'].includes(
          ...
          try {
            ...
            violate(
              `OpenTUI root.renderList cannot be read: ${error instanceof Error ? error.message : String(error)}`,
            );
            return Reflect.apply(originalRootRender, this, [buffer, deltaTime]);
          }
          ...
          try {
              ...
              (original) =>
                function (x: number, y: number, width: number, height: number) {
                  if (pending === current && root.currentRenderable === undefined) {
                    try {
                      current.clipStack.push(
                        intersection(current.clipStack.at(-1)!, rect(x, y, width, height)),
                      );
                    } catch (error) {
                      fail(error instanceof Error ? error.message : String(error));
                    }
            ...
            bufferPop = shadowMethod(
              buffer,
              'popScissorRect',
              (original) =>
                function (...args) {
                  if (pending === current && root.currentRenderable === undefined) {
                    if (current.clipStack.length <= 1)
                      fail('OpenTUI output scissor stack underflow');
            ...
            if (root.renderList !== proxy)
              fail('OpenTUI replaced root.renderList during the observed render pass');
            if (
              result !== null &&
              typeof result === 'object' &&
              typeof (result as { then?: unknown }).then === 'function'
            ...
            if (current.clearCount !== expectedClears) {
              fail(
                `OpenTUI render pass cleared hit-grid scissors ${current.clearCount} times; expected ${expectedClears}`,
              );
            }
            ...
            if (installedBufferPop !== undefined)
              restore('buffer.popScissorRect', () => installedBufferPop.restore());
            if (installedBufferPush !== undefined)
              restore('buffer.pushScissorRect', () => installedBufferPush.restore());
            ...
            restore('root.renderList', () => {
              ...
              if (
                currentRenderList !== undefined &&
                'value' in currentRenderList &&
                currentRenderList.value === proxy
              ) {
  ...
  const frameListener = (event?: { readonly frameId?: number }): void => {
    ...
    try {
      ...
      if (
        ...
        const detail =
          pending?.detail ?? `OpenTUI frame ${String(frameId)} has no complete runtime observation`;
        pending = undefined;
        violate(detail);
        return;
      }
      ...
      violate(
        `OpenTUI runtime commit failed: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  };
  ...
  try {
    ...
    try {
      restoreCapability(renderer, provider, previousCapability);
    } catch {
      /* continue rollback */
    }
    throw error;
  ...
  return {
    ...
    dispose() {
      ...
      try {
        ...
        /* best-effort after hostile mutation */
      }
    },
  };
}
...
function assertRuntimeShape(renderer: RuntimeRenderer): void {
  if (renderer === null || typeof renderer !== 'object')
    throw new Error('OpenTUI renderer is unavailable');
  if (renderer.root === null || typeof renderer.root !== 'object')
    throw new Error('OpenTUI renderer.root is unavailable');
  if (typeof renderer.root.render !== 'function')
    throw new Error('OpenTUI root.render is unavailable');
  if (!Array.isArray(renderer.root.renderList))
    throw new Error('OpenTUI root.renderList is unavailable');
  for (const key of [
    'on',
    'off',
    'requestRender',
  ...
  if (
    renderListDescriptor === undefined ||
    !('value' in renderListDescriptor) ||
    !Array.isArray(renderListDescriptor.value)
  ) {
  ...
  if (!Object.isExtensible(renderer) && !Object.hasOwn(renderer, RUNTIME_FRAME_GEOMETRY_SYMBOL)) {
    throw new Error('OpenTUI renderer cannot expose runtime geometry capability');
  }
  const runtimeCapability = Object.getOwnPropertyDescriptor(
    renderer,
    RUNTIME_FRAME_GEOMETRY_SYMBOL,
  );
...
function restoreCapability(
  ...
  if (previous === undefined)
    delete (renderer as unknown as Record<PropertyKey, unknown>)[RUNTIME_FRAME_GEOMETRY_SYMBOL];
...
function restoreShadows(shadows: ShadowedMethod[]): void {
  for (const shadow of [...shadows].reverse()) {
    try {
      ...
      /* keep restoring the remaining descriptors */
    }
  }
}
...
function identityOf(node: Pick<ObservableNode, 'num'>): string {
  if (!Number.isSafeInteger(node.num) || node.num <= 0)
    throw new Error(`invalid OpenTUI renderable identity ${String(node.num)}`);
...
function finite(value: unknown, label: string): number {
  if (typeof value !== 'number' || !Number.isFinite(value))
    throw new Error(`OpenTUI ${label} is not finite`);
  return value;
}
