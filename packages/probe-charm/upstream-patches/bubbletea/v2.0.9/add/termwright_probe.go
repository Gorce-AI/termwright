...
// renderer. The flush hook already owns cursedRenderer.mu; it must never
...
func (p *termwrightProbeState) rendererState(renderer *cursedRenderer) *termwrightRendererState {
	if renderer == nil {
		return nil
	}
...
func (p *termwrightProbeState) replayLatestFrames() {
	...
	renderers := make([]*cursedRenderer, 0)
	p.renderers.Range(func(key, _ any) bool {
		renderers = append(renderers, key.(*cursedRenderer))
		return true
	})
...
func (p *termwrightProbeState) snapshot(model Model, view View, columns, rows int) (*protocol.Snapshot, annotate.SemanticKey) {
...
func (p *termwrightProbeState) appendOpaque(candidates *[]termwrightCandidate, parentKey, identityPath, fieldName, reason string) {
	...
	*candidates = append(*candidates, termwrightCandidate{
		identityKey: identityPath + "/opaque/" + reason,
		parentKey:   parentKey,
		node: protocol.Node{
			Role: protocol.RoleGeneric, Name: name, FrameworkType: "opaque-container", OpaqueChildren: true,
			Extended: map[string]any{"degradedCapability": "custom-container-enumeration", "opaqueReason": reason},
			P:        protocol.ProvenanceFramework, PX: map[string]string{"role": protocol.ProvenanceRecognizer},
		},
	})
}
