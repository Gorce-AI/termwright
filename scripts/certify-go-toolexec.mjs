...
async function expectFailure(operation, expected, description) {
  try {
    ...
    if (typeof expected === "string") {
      ...
      assert(expected.test(errorText(error)), `${description}: unexpected failure: ${errorText(error)}`);
    }
    return;
  }
...
async function scratch(label) {
  ...
  const directory = join(
    temporaryRoot,
    `${String(scratchIndex).padStart(2, "0")}-${label}`,
  );
...
try {
  ...
  await certify("vendor-mode dependency selection", async () => {
    ...
    assert(await readFile(dependency, "utf8") === before, "vendored dependency source bytes changed");
  });
  ...
  await certify("compiler identity changes with owned content", async () => {
    ...
    assert(second.configDigest !== first.configDigest, "owned content did not change config identity");
    ...
    assert(firstVersion.stdout.includes(first.configDigest), "first compiler identity omitted config digest");
    assert(secondVersion.stdout.includes(second.configDigest), "second compiler identity omitted config digest");
  ...
  await certify("compiler identity ignores materialisation directory", async () => {
    ...
    assert(second.configDigest === first.configDigest, "materialisation path changed config identity");
    const { stdout: toolDir } = await run("go", ["env", "GOTOOLDIR"]);
    const compiler = join(toolDir.trim(), process.platform === "win32" ? "compile.exe" : "compile");
    const firstVersion = await run(first.wrapperFile, [compiler, "-V=full"]);
    const secondVersion = await run(second.wrapperFile, [compiler, "-V=full"]);
    assert(secondVersion.stdout === firstVersion.stdout, "materialisation path changed compiler identity");
  });
  ...
  await certify("compiler identity includes imported archives", async () => {
    ...
    assert(second.configDigest !== first.configDigest, "import archive did not change config identity");
    const { stdout: toolDir } = await run("go", ["env", "GOTOOLDIR"]);
    const compiler = join(toolDir.trim(), process.platform === "win32" ? "compile.exe" : "compile");
    const firstVersion = await run(first.wrapperFile, [compiler, "-V=full"]);
    const secondVersion = await run(second.wrapperFile, [compiler, "-V=full"]);
    assert(secondVersion.stdout !== firstVersion.stdout, "import archive did not change compiler identity");
  });
