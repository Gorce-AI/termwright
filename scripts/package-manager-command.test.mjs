...
describe('portable pnpm command resolution', () => {
  ...
  it('fails closed on Windows instead of returning a .cmd or shell invocation', () => {
    expect(() => pnpmInvocation(['run', 'test'], { env: {}, platform: 'win32' })).toThrow(
      /npm_execpath or PNPM_HOME.*refusing a pnpm\.cmd or shell fallback/u,
    );
  });

  it('uses the ordinary executable name on POSIX', () => {
    expect(pnpmInvocation(['run', 'test'], { env: {}, platform: 'linux' })).toEqual({
      command: 'pnpm',
      args: ['run', 'test'],
    });
  });
  ...
  it('rejects an arbitrary JavaScript file that is not the declared pnpm package bin', () => {
    ...
    expect(() => pnpmInvocation([], { env: { npm_execpath: path }, platform: 'win32' })).toThrow(
      /validated pnpm JavaScript CLI/u,
    );
  });

  it('rejects a command that cannot be passed to execFile exactly', () => {
    expect(() => pnpmInvocation(['test', 1], { env: {} })).toThrow(/array of strings/u);
  });
