...
const targetUrl = new URL('../website/src/content/docs/reference/configuration.md', import.meta.url);
...
export function renderResourceProfiles(profiles) {
  ...
  if (resourceClasses.length === 0) throw new TypeError('resource profiles must declare capacities');

  const header = ['Profile', 'Workers', ...resourceClasses.map(resourceLabel), 'Per terminal'];
  const divider = header.map(() => '---');
  const rows = entries.map(([name, profile]) => {
    if (profile?.name !== name || profile.scheduler?.pool !== 'forks' || profile.scheduler.fileParallelism !== true) {
    ...
    const capacities = resourceClasses.map((resource) => positive(profile.capacities[resource], `${name}.${resource}`));
    const perTerminal = Object.entries(profile.perTerminal ?? {}).map(([resource, amount]) =>
...
async function loadProfiles() {
  ...
  if (errors.length > 0) {
    throw new Error(`cannot load resource profile source: ${errors.map((error) => error.messageText).join('; ')}`);
  }
...
async function main() {
  ...
  const expected = renderDocument(current, renderResourceProfiles(await loadProfiles()), targetPath);
  if (current !== expected) {
    if (process.argv.includes('--write')) await writeFile(targetUrl, expected, 'utf8');
    else throw new Error('resource profile documentation drifted; run node scripts/generate-resource-profile-docs.mjs --write');
  }
  process.stdout.write(process.argv.includes('--write') ? 'wrote resource profile documentation\n' : 'resource profiles: zero drift\n');
}
