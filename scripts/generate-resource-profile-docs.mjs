...
export function renderResourceProfiles(profiles) {
  ...
  const purposes = {
    ...
    ci: 'Linux and macOS CI',
    'windows-ci': 'Windows CI',
    stress: 'Intentional high-fanout capacity tests',
  };
  const rows = entries.map(([name, profile]) => {
    ...
    if (Object.keys(profile.capacities ?? {}).length === 0)
      throw new TypeError(`resource profile ${name} must declare capacities`);
    return [`\`${name}\``, purposes[name] ?? 'Custom scheduling policy', 'Host-derived'];
  });
