...
describe('quality performance timing', () => {
  it('uses logical ordering and monotonic durations when wall time moves backwards', () => {
    const later = manifest({ run: 2, seq: 20, startedAt: 500, finishedAt: 100, durationMs: 80, attemptStart: 30, attemptFinish: 50, attemptDuration: 200 });
    const first = manifest({ run: 1, seq: 10, startedAt: 1_000, finishedAt: 2_000, durationMs: 100, attemptStart: 40, attemptFinish: 65, attemptDuration: 250 });
  ...
  it('fails closed for retries, multiple attempts and mixed host invocations', () => {
    const valid = manifest({
      run: 1, seq: 10, durationMs: 100, attemptStart: 40, attemptFinish: 65, attemptDuration: 25,
    });
    expect(() => summarizeQualityTiming([
      valid,
      { ...manifest({ run: 2, seq: 20 }), attempts: [{ ...manifest({ run: 2, seq: 20 }).attempts[0], retry: 1 }] },
...
function manifest({
  run, seq, startedAt = 0, finishedAt = 0, durationMs = 50, attemptStart = 10,
  attemptFinish = 15, attemptDuration = 5,
}) {
  return {
    ...
    status: 'passed',
    attempts: [{
      status: 'passed', retry: 0, repeat: 0, durationMs: attemptDuration,
      startedAfterRunMs: attemptStart, finishedAfterRunMs: attemptFinish,
    }],
