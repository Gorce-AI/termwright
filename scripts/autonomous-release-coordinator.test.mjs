...
function expandedCheckNames(job) {
  return matrixRows(job.matrix).map((row) => {
    const rendered = job.workflowName
      .replace(
        /\$\{\{ matrix\.python && format\(' \{0\}', matrix\.python\) \|\| '' \}\}/gu,
        row.python === undefined ? '' : ` ${row.python}`,
      )
      .replace(/\$\{\{ matrix\.([A-Za-z0-9_-]+) \}\}/gu, (_expression, axis) => row[axis] ?? '');
    if (rendered.includes('${{'))
      throw new Error(`unsupported CI job-name expression: ${rendered}`);
    return rendered;
  });
}
...
describe('trusted autonomous coordinator', () => {
  ...
  it('rejects a stale default-branch certification SHA', () => {
    const run = {
      name: 'Framework compatibility candidates',
      path: '.github/workflows/upstream-candidates.yml',
      status: 'completed',
      event: 'schedule',
      run_attempt: 1,
    ...
    expect(() =>
      validateTrustedUpstreamRun(run, { repository, defaultBranch, defaultHead: base }),
    ).toThrow(/stale/u);
    expect(() =>
      validateTrustedUpstreamRun(
        { ...run, run_attempt: 2 },
        { repository, defaultBranch, defaultHead: head },
      ),
    ).toThrow(/clean first workflow attempt/u);
  });

  it('rejects malicious paths even when the rest of the compatibility PR is allowed', () => {
    expect(() =>
      validateChangedFiles('compatibility', [
        'packages/probe-ink/src/certified-instrumentation.json',
        'packages/probe-ink/package.json',
        'packages/ink/package.json',
      ]),
    ).not.toThrow();
    expect(() => validateChangedFiles('compatibility', ['clients/python/uv.lock'])).toThrow(
      /forbidden path/u,
    );
    expect(() =>
      validateChangedFiles('compatibility', [
    ...
    expect(() =>
      validateChangedFiles('compatibility', [
        'packages/probe-charm/upstream-patches/../../scripts/pwn.mjs',
      ]),
    ).toThrow(/forbidden path/u);
    expect(() =>
      validateChangedFiles('heartbeat', ['compatibility/workflow-heartbeat.json']),
    ).not.toThrow();
    expect(() => validateChangedFiles('heartbeat', ['compatibility/registry.json'])).toThrow(
      /forbidden path/u,
    );
  });
  ...
  it('refreshes a source-bound heartbeat only every 30 days and rejects time rollback', () => {
    ...
    expect(JSON.parse(first)).toEqual({
      schemaVersion: 1,
      kind: 'termwright-schedule-heartbeat',
      sourceRunId: '123',
      sourceRevision: head,
      observedAt: run.created_at,
    });
    expect(
      nextHeartbeatRecord({ ...run, id: 124, created_at: '2026-02-28T00:00:00.000Z' }, first),
    ).toBeNull();
    expect(
      nextHeartbeatRecord({ ...run, id: 125, created_at: '2026-03-02T00:00:00.000Z' }, first),
    ).not.toBeNull();
    expect(() =>
  ...
  it('rejects symlinks and submodules hidden behind allowed changed paths', () => {
    ...
    expect(() =>
      validateChangedFileObjects(files, {
        tree: [{ path: 'compatibility/registry.json', mode: '120000', type: 'blob', size: 12 }],
      }),
    ).toThrow(/regular file/u);
    expect(() =>
      validateChangedFileObjects(files, {
        tree: [{ path: 'compatibility/registry.json', mode: '160000', type: 'commit', size: 12 }],
      }),
    ).toThrow(/regular file/u);
  });
  ...
  it('requires every exact CI job and rejects missing checks', () => {
    ...
    expect(() =>
      validateRequiredCiJobs([
        ...CI_JOBS.map((name) => ({ name, conclusion: 'success' })),
        { name: 'unreviewed new gate', conclusion: 'success' },
      ]),
    ).toThrow(/unexpected jobs/u);
  });

  it('keeps the release authorization contract synchronized with every ci.yml job and matrix', async () => {
    const workflow = ciWorkflowSurface(
      await readFile(new URL('../.github/workflows/ci.yml', import.meta.url), 'utf8'),
    );
    const contract = Object.fromEntries(
      Object.entries(CI_JOB_CONTRACT).map(([id, job]) => [
        id,
        {
  ...
  it('accepts only the exact automation PR identity and file set', () => {
    ...
    expect(() =>
      validateAutomationPr(
        ...
        [
          {
            filename: 'compatibility/registry.json',
    ...
    const heartbeatPr = {
      ...
      head: { ...pr.head, ref: 'automation/workflow-heartbeat' },
    };
    expect(() =>
      validateAutomationPr(
        'heartbeat',
  ...
  it('deduplicates an exact release dispatch', () => {
    ...
    expect(
      shouldDispatchRelease([{ event: 'workflow_dispatch', display_title: title }], title),
    ).toBe(false);
    expect(
      shouldDispatchRelease(
        [{ event: 'workflow_dispatch', display_title: `${title}-other` }],
        title,
      ),
    ).toBe(true);
  });
  ...
  it('blocks a new merge until the latest trusted release state is successful', () => {
    const run = {
      ...
      status: 'completed',
      conclusion: 'failure',
      display_title: `Release publish main @ ${head} (Version PR 42)`,
      head_sha: head,
      head_branch: defaultBranch,
    ...
    expect(() => assertReleaseStateQuiescent([run], { repository, defaultBranch })).toThrow(
      /refusing to advance/u,
    );
    expect(() =>
      assertReleaseStateQuiescent([{ ...run, status: 'in_progress', conclusion: null }], {
        repository,
        defaultBranch,
      }),
    ).toThrow(/refusing to advance/u);
    expect(() =>
      assertReleaseStateQuiescent([{ ...run, conclusion: 'success' }], {
        repository,
        defaultBranch,
      }),
    ).not.toThrow();
  ...
  it('requires strict, non-bypassable branch protection with the exact CI gate set', () => {
    const protection = {
      required_status_checks: {
        strict: true,
        contexts: REQUIRED_BRANCH_CHECKS,
        checks: REQUIRED_BRANCH_CHECKS.map((context) => ({ context, app_id: 15368 })),
      },
    ...
    ).toThrow(/zero approving reviews/u);
    expect(() =>
      validateBranchProtection({
        ...protection,
        required_pull_request_reviews: {
      ...
      validateBranchProtection({
        ...
        required_status_checks: {
          ...
          checks: [
            ...
            { context: 'unreviewed', app_id: 15368 },
          ],
        },
      }),
    ).toThrow(/unexpected required CI check set/u);
    expect(() =>
      validateBranchProtection({
        ...protection,
        required_status_checks: {
          ...protection.required_status_checks,
          checks: protection.required_status_checks.checks.map((check, index) =>
  ...
  it('requires exactly one numeric source-run attestation on a compatibility PR', () => {
    expect(
      compatibilitySourceRunId(
        'Automated compatibility certification.\n\nSource certification run: 12345',
      ),
    ).toBe('12345');
    expect(() => compatibilitySourceRunId('no source')).toThrow(/exactly one/u);
    expect(() =>
      compatibilitySourceRunId('Source certification run: 1\nSource certification run: 2'),
  ...
  it('accepts only a failed SHA-bound Release run for intervention', () => {
    ...
    expect(validateFailedReleaseRun(run, { repository, defaultBranch })).toMatchObject({
      mode: 'publish',
      sha: head,
      versionPr: '42',
    });
    expect(() =>
      validateFailedReleaseRun({ ...run, conclusion: 'success' }, { repository, defaultBranch }),
    ).toThrow(/does not require intervention/u);
    expect(() =>
      validateFailedReleaseRun(
        { ...run, display_title: `Release publish main @ ${base} (Version PR 42)` },
        { repository, defaultBranch },
      ),
    ).toThrow(/not bound/u);
    expect(() =>
