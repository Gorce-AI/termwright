...
function matrixRows(matrix) {
  ...
  return Object.entries(matrix).reduce((rows, [axis, values]) => rows.flatMap((row) => values.map((value) => ({ ...row, [axis]: value }))), [{}]);
}
...
describe('trusted autonomous coordinator', () => {
  ...
  it('rejects malicious paths even when the rest of the compatibility PR is allowed', () => {
    expect(() => validateChangedFiles('compatibility', ['packages/probe-ink/src/certified-instrumentation.json', 'packages/probe-ink/package.json', 'packages/ink/package.json'])).not.toThrow();
    expect(() => validateChangedFiles('compatibility', ['clients/python/uv.lock'])).toThrow(/forbidden path/u);
    expect(() =>
    ...
    expect(() => validateChangedFiles('compatibility', ['packages/probe-charm/upstream-patches/../../scripts/pwn.mjs'])).toThrow(/forbidden path/u);
  ...
  it('accepts only the exact automation PR identity and file set', () => {
    ...
    expect(() =>
      validateAutomationPr('compatibility', pr, [{ filename: 'compatibility/certified-upstreams.json', status: 'modified' }], { repository, defaultBranch, headSha: head, defaultHead: base }),
    ).not.toThrow();
    expect(() =>
      validateAutomationPr('compatibility', { ...pr, user: { login: 'attacker' } }, [{ filename: 'compatibility/certified-upstreams.json', status: 'modified' }], {
        repository,
        defaultBranch,
        headSha: head,
        defaultHead: base,
    ...
    expect(() =>
      validateAutomationPr('compatibility', { ...pr, changed_files: 2 }, [{ filename: 'compatibility/certified-upstreams.json', status: 'modified' }], {
        repository,
        defaultBranch,
        headSha: head,
        defaultHead: base,
    ...
    expect(() =>
      validateAutomationPr('compatibility', pr, [{ filename: 'compatibility/registry.json', previous_filename: '.github/workflows/release.yml', status: 'renamed' }], {
        repository,
        defaultBranch,
        headSha: head,
        defaultHead: base,
    ...
    expect(() =>
      validateAutomationPr('heartbeat', heartbeatPr, [{ filename: 'compatibility/workflow-heartbeat.json', status: 'modified' }], { repository, defaultBranch, headSha: head, defaultHead: base }),
    ).not.toThrow();
  });
  ...
  it('recognizes only ordinary safe pending changeset files', () => {
    expect(pendingChangesetFiles(['README.md', 'feature-one.md', 'fix_2.md', '.fake.md', '../escape.md', 'nested/file.md', 'not-markdown.txt'])).toEqual(['feature-one.md', 'fix_2.md']);
  });
  ...
  it('requires strict, non-bypassable branch protection with the exact CI gate set', () => {
    const protection = {
      required_status_checks: { strict: true, contexts: REQUIRED_BRANCH_CHECKS, checks: REQUIRED_BRANCH_CHECKS.map((context) => ({ context, app_id: 15368 })) },
    ...
    expect(() =>
      validateBranchProtection({ ...protection, required_pull_request_reviews: { ...protection.required_pull_request_reviews, bypass_pull_request_allowances: { users: [], teams: [], apps: [{}] } } }),
    ).toThrow(/bypass/u);
    expect(() => validateBranchProtection({ ...protection, required_pull_request_reviews: { ...protection.required_pull_request_reviews, required_approving_review_count: 1 } })).toThrow(
      /zero approving reviews/u,
    );
    ...
    expect(() =>
      validateBranchProtection({
        ...protection,
        required_status_checks: { ...protection.required_status_checks, checks: [...protection.required_status_checks.checks, { context: 'unreviewed', app_id: 15368 }] },
      }),
    ).toThrow(/unexpected required CI check set/u);
    expect(() =>
      validateBranchProtection({
        ...protection,
        required_status_checks: { ...protection.required_status_checks, checks: protection.required_status_checks.checks.map((check, index) => (index === 0 ? { ...check, app_id: 1 } : check)) },
      }),
