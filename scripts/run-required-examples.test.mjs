...
import {
  requiredExamples,
  requiredExampleArguments,
  runRequiredExamples,
} from './run-required-examples.mjs';
...
describe('required public examples', () => {
  ...
  it('keeps recursive workspace filters portable and non-vacuous', async () => {
    ...
    expect(
      manifest.scripts['check:local'].indexOf('--filter=./examples/* --fail-if-no-match run build'),
    ).toBeLessThan(
      manifest.scripts['check:local'].indexOf('pnpm test -- --resource-profile local'),
    );
    expect(manifest.scripts['check:local']).toContain('pnpm docs:api');
    expect(manifest.scripts['check:local']).toContain(
      'git diff --exit-code -- website/src/content/docs/api',
    );
    expect(
      `${manifest.scripts.build}\n${manifest.scripts.typecheck}\n${manifest.scripts['check:local']}`,
