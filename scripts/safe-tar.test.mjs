...
describe('safe registry tar extraction', () => {
  it('extracts only normalized regular files after stripping the package root', async () => {
    ...
    await safeExtractTarGz(
      archive(
        header('crate-1.0.0/', Buffer.alloc(0), '5'),
        header('crate-1.0.0/src/lib.rs', Buffer.from('pub fn ok() {}\n')),
      ),
      destination,
      { stripComponents: 1 },
    );
  ...
  it.each([
    ...
    const destination = await mkdtemp(join(tmpdir(), 'tw-safe-tar-'));
    await expect(safeExtractTarGz(archive(entry), destination)).rejects.toThrow(
      /unsafe tar archive/u,
    );
  });
  ...
  it('rejects duplicate paths and decompression bombs within explicit limits', async () => {
    ...
    await expect(
      safeExtractTarGz(
        archive(header('same', Buffer.from('a')), header('same', Buffer.from('b'))),
        destination,
      ),
    ).rejects.toThrow(/duplicate path/u);
    await expect(
      safeExtractTarGz(archive(header('large', Buffer.alloc(2048))), destination, {
        maxBytes: 1024,
      }),
    ).rejects.toThrow(/exceeded 1024/u);
  });
  ...
  it('rejects a regular file used as another entry parent before extraction', async () => {
    ...
    await expect(
      safeExtractTarGz(
        archive(
          header('crate/src', Buffer.from('file')),
          header('crate/src/lib.rs', Buffer.from('nested')),
        ),
        destination,
        { stripComponents: 1 },
      ),
    ).rejects.toThrow(/regular file is parent/u);
  });
  ...
  it('rejects a second archive hidden after the end marker', async () => {
    ...
    const concatenated = gzipSync(
      Buffer.concat([
        header('safe', Buffer.from('ok')),
        Buffer.alloc(1024),
        header('../outside', Buffer.from('bad')),
      ]),
    );
    await expect(safeExtractTarGz(concatenated, destination)).rejects.toThrow(
      /trailing or concatenated/u,
    );
  });
});
