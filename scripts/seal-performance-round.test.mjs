...
describe('performance round seal', () => {
  ...
  it('rejects a wrong sequence and foreign, missing or later-attempt CI provenance', async () => {
    ...
    ).rejects.toThrow(/expected subject, round, sequence or SHA/u);
    await expect(
      loadPerformanceRoundSeal(
        path,
        {
    ...
    ).rejects.toThrow(/current workflow run/u);
    ...
    ).rejects.toThrow(/provenance is missing/u);
    ...
    ).rejects.toThrow(/not the first attempt/u);
  });
});
...
async function reports() {
  ...
  await Promise.all(
    Object.entries(PERFORMANCE_ROUND_INPUTS).map(([name, file]) =>
