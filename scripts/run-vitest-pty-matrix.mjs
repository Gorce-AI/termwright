...
import {
  hasClosedChannelDiagnostic,
  isVitestPtyCellFailure,
} from './test-support/vitest-pty-diagnostics.mjs';
...
if (embeddedVitest !== '4.1.11') {
  throw new Error(
    `reliability matrix requires exact embedded Vitest 4.1.11, observed ${String(embeddedVitest)}`,
  );
}
...
try {
  for (const version of versions) {
    ...
    const driverBackend = pathToFileURL(
      join(root, 'packages', 'driver', 'dist', 'experimental.js'),
    ).href;
    await writeFile(
      join(project, 'driver-backend.mjs'),
      `export { createNativePtyBackend } from ${JSON.stringify(driverBackend)};\n`,
    );
    await writeFile(
    ...
    for (const pool of pools)
      for (const workers of workerCounts)
        for (const terminals of ptyConcurrency) {
          for (const parallelFiles of fileParallelism) {
            const telemetry = join(
              project,
              `telemetry-${pool}-${workers}-${terminals}-files-${parallelFiles}`,
            );
            ...
            try {
              ...
              code =
                typeof error === 'object' && error !== null && typeof error.code === 'number'
                  ? error.code
                  : 1;
              stdout =
                typeof error === 'object' && error !== null && typeof error.stdout === 'string'
                  ...
                  : '';
              stderr =
                typeof error === 'object' && error !== null && typeof error.stderr === 'string'
                  ? error.stderr
                  : String(error);
            }
            const { records, errors: telemetryReadErrors } = await readTelemetryShards(
              telemetry,
              parallelFiles,
            );
            const telemetryVerdict = validateVitestPtyTelemetry(records, {
              ...
              arch: process.arch,
              readErrors: telemetryReadErrors,
            });
            results.push({
              vitest: version,
              node: process.version,
              ...
              workersObserved: [
                ...new Set(
                  records.map((record) => `${String(record.pid)}:${String(record.threadId)}`),
                ),
              ].sort(),
  ...
  await writeFile(
    output,
    `${JSON.stringify({ v: 1, generatedAt: new Date().toISOString(), results }, null, 2)}\n`,
  );
...
console.log(
  `Vitest/PTy matrix wrote ${results.length} cells to ${output}; certified failures: ${certifiedFailures.length}`,
);
...
async function readTelemetryShards(directory, parallelFiles) {
  ...
  const errors = [];
  const sources = [
    'vitest-pty-pressure.test.mjs',
    ...Array.from(
      { length: filesPerCell - 1 },
      (_, index) => `vitest-pty-pressure-${index + 1}.test.mjs`,
    ),
  ];
  const expectedNames = new Set(
    sources.flatMap((source) => [`${source}.jsonl`, ...(parallelFiles ? [`${source}.ready`] : [])]),
  );
...
function validateCertifiedMatrix(entries) {
  ...
  for (const pool of ['forks', 'threads'])
    for (const workers of [1, 2, 4]) {
      for (const terminals of [1, 2, 4, 8])
        for (const fileParallelism of [true, false]) {
  ...
  if (
    certified.length !== expected.size ||
    actual.size !== expected.size ||
    [...expected].some((key) => !actual.has(key))
  ) {
    throw new Error(
      `certified Vitest matrix is incomplete: expected ${expected.size} exact cells, observed ${actual.size}`,
    );
  }
}
