  ...
  return gzipSync(
    Buffer.concat([
      ...Object.entries(files)
        ...
        .map(([path, contents]) =>
          tarEntry(`package/${path}`, Buffer.from(contents)),
        ),
      Buffer.alloc(1024),
    ]),
  );
...
async function npmClosureFixture() {
  ...
  await writeFile(
    join(rootPackage, "node_modules", ".bin", "dependency"),
    "#!/bin/sh\n",
  );
...
describe("framework candidate evidence binding", () => {
  it("binds generated tview candidates to the repository-owned Go client before tidy", async () => {
    ...
    try {
      const canonicalClient = await bindLocalTermwrightGoClient(
        app,
        process.env,
        client,
      );
      const edited = JSON.parse(
        (await exec("go", ["mod", "edit", "-json"], { cwd: app })).stdout,
      );
  ...
  it("rejects a generated patch bundle changed by candidate code after trusted preparation", async () => {
    ...
    await writeFile(
      join(directory, "patch", "manifest.json"),
      '{"version":1}\n',
    );
    ...
    await expect(
      verifyPreparedUpdateInvariant(invariant),
    ).resolves.toBeUndefined();
    await writeFile(
      join(directory, "patch", "manifest.json"),
      '{"version":2}\n',
    );
    await expect(verifyPreparedUpdateInvariant(invariant)).rejects.toThrow(
      /patch tree changed/u,
    );
    await writeFile(
      join(directory, "patch", "manifest.json"),
      '{"version":1}\n',
  ...
  it("walks the real pnpm package location when dependency versions diverge", async () => {
    ...
    const packageDirectory = (key, name) =>
      join(directory, ".pnpm", key, "node_modules", name);
    ...
    try {
      ...
      await symlink(
        stripAnsi71,
        join(rootPackage, "node_modules", "strip-ansi"),
        "dir",
      );
      await symlink(
        stripAnsi72,
        join(dirname(stringWidth), "strip-ansi"),
        "dir",
      );
      ...
      await expect(
        installedDependencyFrom(child.directory, "strip-ansi"),
  ...
  it("cryptographically binds the exact installed npm graph to every discovered tarball", async () => {
    const { candidate, directory, fetchImpl, probe } =
      await npmClosureFixture();
    try {
      await expect(
  ...
  it("rejects altered installed bytes even when package names, versions and declarations match", async () => {
    const { candidate, dependency, directory, fetchImpl, probe } =
      await npmClosureFixture();
    try {
      await writeFile(
  ...
  it("rejects undeclared installed content below the package node_modules boundary", async () => {
    const { candidate, directory, fetchImpl, probe, rootPackage } =
      await npmClosureFixture();
    try {
      await mkdir(join(rootPackage, "node_modules"), { recursive: true });
      await writeFile(
      ...
      await expect(
        verifyInstalledNpmClosure(candidate, probe, { fetchImpl }),
      ).rejects.toThrow(/root content does not match/u);
    } finally {
      await rm(directory, { recursive: true, force: true });
    }
  });
  ...
  it("rejects undeclared package-manager launchers below node_modules/.bin", async () => {
    ...
    try {
      ...
      await writeFile(
        join(rootPackage, "node_modules", ".bin", "unbound"),
        "#!/bin/sh\n",
      );
      await expect(
        verifyInstalledNpmClosure(candidate, probe, { fetchImpl }),
      ).rejects.toThrow(/bin entry is undeclared/u);
    } finally {
      await rm(directory, { recursive: true, force: true });
    }
  });
  ...
  it("rejects expected closure nodes that are unreachable from the installed root", async () => {
    ...
    try {
      ...
      await expect(
        verifyInstalledNpmClosure(candidate, probe, { fetchImpl }),
      ).rejects.toThrow(/unreachable nodes: orphan@9\.0\.0/u);
    } finally {
      await rm(directory, { recursive: true, force: true });
    }
  });
  ...
  it("rejects a session whose frozen contract lacks semantic support", async () => {
    ...
    await expect(
      assertCandidateSemanticSession(session, "bubbletea-v2@v2.0.9"),
    ).rejects.toThrow(/no supported semantic tree/u);
  });
