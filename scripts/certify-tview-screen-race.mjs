...
const temporaryRoot = await realpath(
  await mkdtemp(join(tmpdir(), "tw-tview-screen-race-")),
);
let terminal;
...
try {
  ...
  const resolved = await run(
    "go",
    ["mod", "download", "-json", "github.com/rivo/tview"],
    { cwd: fixtureApp },
  );
  const module = JSON.parse(resolved.stdout);
  if (typeof module.Dir !== "string" || module.Dir === "") {
    throw new Error(
      "go mod download did not return the tview source directory",
    );
  }
