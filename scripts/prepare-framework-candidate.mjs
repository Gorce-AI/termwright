...
export function assertArtifactSha256(bytes, expected, label) {
  ...
  const normalized =
    typeof expected === 'string' && expected.startsWith('sha256:')
      ? expected
      : `sha256:${String(expected)}`;
  if (actual !== normalized)
    throw new Error(`${label}: downloaded archive hashes ${actual}, expected ${normalized}`);
}
  ...
  const visit = async (current) => {
    for (const entry of (await readdir(current, { withFileTypes: true })).sort((a, b) =>
      ...
      if (entry.isDirectory()) await visit(path);
        ...
        throw new Error('candidate patch bundles may contain only directories and regular files');
    }
  };
...
async function latestTemplate(rootDir, patchRoot, candidateVersion) {
  ...
  const sameLine = versions.filter(
    (entry) =>
      candidateMajor === undefined || parseVersion(entry.version)?.major === candidateMajor,
  );
  sameLine.sort((a, b) => compareVersions(b.version, a.version));
  if (sameLine.length === 0)
    throw new Error(
      `no audited transform template exists for ${candidateVersion} below ${patchRoot}`,
    );
  return sameLine[0];
}
...
async function preparePatchBundleInner({
  rootDir = root,
  candidate,
  sourceRoot,
  outputDirectory,
  ...
  if (candidate.mode !== 'patch' || candidate.patch?.status !== 'needs-patch')
    throw new Error(`${candidate.id}: candidate does not need a generated patch`);
  if (typeof sourceRevision !== 'string' || sourceRevision.length < 7)
    throw new Error('sourceRevision is required');
  const streamRoot = safeRelative(candidate.patch.path, 'candidate.patch.path')
    .split('/')
    .slice(0, -2)
    .join('/');
  ...
  for (const [index, entry] of manifest.patched.entries()) {
    ...
    try {
      await exec(
        'git',
        ['apply', '--unsafe-paths', '-p1', `--directory=.`, join(destination, patch)],
        {
          cwd: sourceRoot,
        },
      );
      ...
      throw new Error(
        `${candidate.id}: audited transform no longer applies to ${target}: ${error.stderr || error.message}`,
      );
    }
...
export async function preparePatchBundles(requests) {
  if (!Array.isArray(requests) || requests.length === 0)
    throw new TypeError('at least one patch bundle request is required');
  for (const request of requests) {
    ...
    if (typeof request.sourceRevision !== 'string' || request.sourceRevision.length < 7)
      throw new Error('sourceRevision is required');
  }
    ...
    if (candidate.registry === 'go') {
      const result = JSON.parse(
        (
          await exec(
            'go',
            ['mod', 'download', '-json', `${candidate.package}@${candidate.version}`],
            {
      ...
      const response = await fetch(
        `https://crates.io/api/v1/crates/${encodeURIComponent(candidate.package)}/${encodeURIComponent(candidate.version)}/download`,
        {
      ...
      if (!response.ok)
        throw new Error(
          `${candidate.id}: crates.io source download failed with ${response.status}`,
        );
      ...
      throw new Error(`${candidate.id}: patch preparation does not support ${candidate.registry}`);
...
export async function removeMaterializedCandidateSource(lease) {
  if (typeof lease === 'object' && lease !== null && disposedMaterializedSourceLeases.has(lease))
    return;
  const directory =
  ...
  if (directory === undefined)
    throw new Error('refusing to remove a source tree not owned by Termwright');
...
export function proposeCompatibilityUpdate(registry, candidate, manifest) {
  ...
  if (!Array.isArray(patchSets))
    throw new Error(`${candidate.id}: certified patch-set declarations are missing`);
  const existing = patchSets.find(
  ...
  else if (existing.patchSetVersion !== manifest.patchSetVersion)
    throw new Error(`${candidate.id}: patch-set declaration conflicts with the generated manifest`);
...
export function recordExecutableVariant(registry, candidate, resolution) {
  ...
  if (framework === undefined)
    throw new Error(`${candidate.id}: compatibility framework row is missing`);
  if (
    typeof resolution?.frameworkVersion !== 'string' ||
    resolution.frameworkVersion.length === 0 ||
    !Array.isArray(resolution.modules) ||
    resolution.modules.length === 0
  ) {
    throw new Error(
      `${candidate.id}: behavioral certification produced no executable module resolution`,
    );
  }
  ...
  if (new Set(modules.map((module) => module.name)).size !== modules.length)
    throw new Error(`${candidate.id}: executable variant resolves one module more than once`);
  if (
    !modules.some(
      (module) => module.name === candidate.package && module.version === candidate.version,
    )
  )
    throw new Error(`${candidate.id}: executable variant does not contain the exact candidate`);
  const previouslyCertifiedModules = new Set(
    framework.instrumentation.variants.flatMap((variant) =>
      variant.modules.map((module) => `${module.name}@${module.version}`),
    ),
  );
  for (const module of modules) {
    ...
    const exactPatchModule = framework.instrumentation.patchSets.some(
      (patch) => patch.name === module.name && patch.version === module.version,
    );
    const certifiedCapabilityCompanion =
      framework.versions.policy === 'capability' &&
      previouslyCertifiedModules.has(`${module.name}@${module.version}`);
    if (!candidateCapabilityModule && !exactPatchModule && !certifiedCapabilityCompanion) {
      throw new Error(
        `${candidate.id}: executable variant uses uncertified module ${module.name}@${module.version}`,
      );
    }
  }
  ...
  const id =
    `${candidate.frameworkId}-${resolution.frameworkVersion}-${createHash('sha256').update(signature).digest('hex').slice(0, 12)}`.replace(
      /[^A-Za-z0-9._-]/gu,
      '-',
    );
  ...
  const isPrimaryFrameworkCandidate =
    framework.frameworkPackage === candidate.package || primaryModules.has(candidate.package);
  if (
    isPrimaryFrameworkCandidate &&
    resolution.frameworkVersion === candidate.version &&
    !framework.versions.verified.includes(candidate.version)
  ) {
    ...
    if (framework.versions.policy === 'exact')
      framework.versions.declared = framework.versions.verified.join(' or ');
  }
  if (
    !Array.isArray(framework.certification?.ids) ||
    typeof framework.probe?.packageVersion !== 'string'
  ) {
