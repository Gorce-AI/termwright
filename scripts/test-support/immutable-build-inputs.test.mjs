...
describe('immutable workspace build inputs', () => {
  ...
  it('accepts a fresh artifact without mutating it', async () => {
    ...
    await expect(
      requireImmutableBuildInputs([built.entry, built.entry], {
  ...
  it('rejects a stale source even when the expected artifact still exists', async () => {
    ...
    await expect(
      requireImmutableBuildInputs([built.entry], {
        label: 'fixture package',
        root: built.root,
        manifestPath: built.manifestPath,
  ...
  it.each(['darwin', 'linux', 'win32'])(
    'permits a declaratively optional %s .node artifact to be absent',
    async (optionalPlatform) => {
  ...
  it('rejects a declared production path that traverses outside its package', async () => {
    await expect(workspace({ files: ['../outside-runtime'] })).rejects.toThrow(
      /outside immutable build root/u,
    );
  });
  ...
  it('names missing and unrecorded inputs without invoking a builder', async () => {
    ...
    await expect(
      requireImmutableBuildInputs([first, second], {
        label: 'fixture package',
        root: built.root,
        manifestPath: built.manifestPath,
      ...
      new RegExp(
        `fixture package.*${escapeRegExp(first)}.*${escapeRegExp(second)}.*pnpm --filter fixture build.*must not build or clean`,
        'u',
      ),
    );
  });
