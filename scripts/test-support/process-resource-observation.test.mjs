...
describe('process resource observation', () => {
  it('retains process start identity independently of PID', () => {
    ...
    expect(sameProcessGeneration(table.get(13), { ...table.get(13), command: 'node replacement.js' })).toBe(true);
    expect(sameProcessSet(
      { pids: [12, 13], table },
  ...
  it('fails closed when footprint omits a requested process or aggregate', () => {
    expect(() => parseDarwinFootprint('node [12]: 64-bit Footprint: 700 B', [12, 13]))
      .toThrow(/one complete aggregate/u);
    expect(() => parseDarwinFootprint('node [12]: 64-bit Footprint: 700 B\nnode [13]: 64-bit Footprint: 800 B', [12, 13]))
      .toThrow(/one complete aggregate/u);
  });
