export function validateVitestPtyTelemetry(records, expected) {
  ...
  for (const record of records) {
    ...
    if (
      ...
      record.threadId < 0 ||
      record.node !== expected.node ||
      record.platform !== expected.platform ||
      record.arch !== expected.arch ||
      !Number.isFinite(record.timeMs) ||
      !Number.isSafeInteger(record.activePtys) ||
      record.activePtys < 0 ||
      !record.memory ||
      !Number.isFinite(record.memory.rss) ||
      record.memory.rss <= 0
    ) {
    ...
    if (
      ...
      errors.push(
        `${source}:${index}:finish lacks a complete READY -> release -> DONE -> exit lifecycle ` +
  ...
  if (transitions.size !== expected.files * expected.casesPerFile) {
    errors.push(
      `expected ${expected.files * expected.casesPerFile} PTY identities, observed ${transitions.size}`,
    );
  }
  ...
  for (const [source, sourceRecords] of bySource) {
    ...
    for (const record of [...sourceRecords].sort((left, right) => left.timeMs - right.timeMs)) {
      ...
      if (record.activePtys !== active)
        errors.push(`${source} reports inconsistent active PTY count`);
    }
    ...
    if (peak !== Math.min(expected.terminals, expected.casesPerFile)) {
      errors.push(
        `${source} reached ${peak} active PTYs, expected ${Math.min(expected.terminals, expected.casesPerFile)}`,
      );
    }
    ...
    ...intervals.map(
      (interval) =>
        new Set(
          intervals
            .filter(
              (candidate) => candidate.start <= interval.start && candidate.end >= interval.start,
            )
