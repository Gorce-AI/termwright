...
export function assertRustTestDiscovered(stdout, test, candidateId) {
  if (!String(stdout).split(/\r?\n/u).includes(`${test}: test`)) {
    throw new Error(
      `${candidateId}: certification test ${test} was not discovered by the Rust test harness`,
    );
  }
}
...
async function packageRootForEntry(entry, expectedName) {
  ...
  for (;;) {
    ...
    if (parent === directory)
      throw new Error(
        `installed npm package ${expectedName} has no reachable package.json`,
      );
    directory = parent;
  }
}
...
export async function installedDependencyFrom(parentDirectory, dependencyName) {
  ...
  for (;;) {
    ...
    const parent = dirname(directory);
    if (parent === directory)
      throw new Error(`installed npm graph cannot resolve ${dependencyName}`);
    directory = parent;
  }
}
...
async function packageContentDigest(directory) {
  ...
  const visit = async (current) => {
    for (const entry of (await readdir(current, { withFileTypes: true })).sort(
      ...
      if (
        entry.isDirectory() &&
        relative(directory, path).split(sep).join("/") === "node_modules/.bin"
      )
        continue;
      if (entry.isDirectory()) await visit(path);
        ...
        throw new Error(
          `installed npm package contains a non-regular entry: ${path}`,
        );
    }
  };
...
function declaredBinNames(manifest) {
  if (typeof manifest.bin === "string")
    return [String(manifest.name).split("/").at(-1)];
  if (
    manifest.bin !== null &&
...
async function verifyMaterializedBinLaunchers(
  packageDirectory,
  expectedNames,
  candidateId,
) {
  ...
  for (const entry of entries) {
    if (!entry.isFile() && !entry.isSymbolicLink()) {
      throw new Error(
        `${candidateId}: installed package-manager bin entry is not a file: ${entry.name}`,
      );
    }
    const baseName = entry.name.replace(/\.(?:cmd|ps1)$/u, "");
    if (!expected.has(baseName)) {
      throw new Error(
        `${candidateId}: installed package-manager bin entry is undeclared: ${entry.name}`,
      );
    }
    seen.add(baseName);
  }
  ...
  if (missing.length > 0) {
    throw new Error(
      `${candidateId}: installed package-manager bin entries are missing: ${missing.join(", ")}`,
    );
  }
}
...
async function verifiedNpmPackageDigest(source, fetchImpl) {
  ...
  if (!response.ok)
    throw new Error(
      `${source.tarball}: artifact download failed with ${response.status}`,
    );
  const bytes = Buffer.from(await response.arrayBuffer());
  if (
    createHash("sha512").update(bytes).digest("base64") !==
    source.integrity.slice("sha512-".length)
  ) {
    throw new Error(
      `${source.tarball}: sha512 did not match discovered evidence`,
    );
  }
  if (sha256(bytes) !== source.tarballSha256)
    throw new Error(
      `${source.tarball}: sha256 did not match discovered evidence`,
    );
...
export async function verifyInstalledNpmClosure(
  candidate,
  probeDirectory,
  { fetchImpl = fetch } = {},
) {
  ...
  const expectedNodes = new Map(
    candidate.source.dependencyClosure.map((node) => [
  ...
  if (rootPackage.manifest.version !== candidate.version) {
    throw new Error(
      `${candidate.id}: installed root is ${rootPackage.manifest.version}, expected ${candidate.version}`,
    );
  }
  const rootExpectedDigest = await verifiedNpmPackageDigest(
    candidate.source,
    fetchImpl,
  );
  if (
    (await packageContentDigest(rootPackage.directory)) !== rootExpectedDigest
  ) {
    throw new Error(
      `${candidate.id}: installed root content does not match the checksum-verified tarball`,
    );
  }
  ...
  const visitExpected = (edges) => {
    for (const edge of edges) {
      ...
      if (expected === undefined)
        throw new Error(
          `${candidate.id}: expected npm closure contains an unresolved edge to ${key}`,
        );
  ...
  const unreachableExpected = [...expectedNodes.keys()].filter(
    (key) => !reachableExpected.has(key),
  );
  if (unreachableExpected.length > 0)
    throw new Error(
      `${candidate.id}: expected npm closure contains unreachable nodes: ${unreachableExpected.join(", ")}`,
    );
  ...
  const visit = async (parent, expectedEdges) => {
    ...
    const normalizedExpected = expectedEdges
      .map(
        ({ packageName: _packageName, version: _version, ...declaration }) =>
          declaration,
      )
      .sort(
        (a, b) => a.name.localeCompare(b.name) || a.type.localeCompare(b.type),
    ...
    if (
      canonicalJson(actualDeclarations) !== canonicalJson(normalizedExpected)
    ) {
      throw new Error(
        `${candidate.id}: installed declarations changed for ${parent.manifest.name}@${parent.manifest.version}`,
      );
    }
    for (const edge of expectedEdges) {
      ...
      try {
        ...
        throw new Error(
          `${candidate.id}: installed graph cannot resolve ${edge.name} from ${parent.manifest.name}`,
          { cause: error },
        );
      }
      if (child.manifest.name !== edge.packageName) {
        throw new Error(
          `${candidate.id}: installed ${edge.name} resolved package ${String(child.manifest.name)}, expected ${edge.packageName}`,
        );
      }
      if (child.manifest.version !== edge.version) {
        throw new Error(
          `${candidate.id}: installed ${edge.name} resolved ${child.manifest.version}, expected ${edge.version}`,
        );
      }
      for (const name of declaredBinNames(child.manifest))
        expectedBinNames.add(name);
      const key = `${child.manifest.name}@${child.manifest.version}`;
      const expected = expectedNodes.get(key);
      if (expected === undefined)
        throw new Error(
          `${candidate.id}: installed graph contains unbound ${key}`,
        );
      if (!visitedNodes.has(key)) {
        const expectedDigest = await verifiedNpmPackageDigest(
          expected,
          fetchImpl,
        );
        if ((await packageContentDigest(child.directory)) !== expectedDigest) {
          throw new Error(
            `${candidate.id}: installed ${key} content does not match the checksum-verified tarball`,
          );
        }
    ...
    await verifyMaterializedBinLaunchers(
      parent.directory,
      expectedBinNames,
      candidate.id,
    );
...
export async function deriveHookInstrumentationProfile(
  candidate,
  archiveBytes,
  sourceRevision,
) {
  if (
    candidate.frameworkId !== "ink" ||
    candidate.hookStrategy !== "exact-source"
  )
    throw new Error(
      `${candidate.id}: no deterministic exact-source hook profile generator`,
    );
...
export function verifyCandidateEvidence(
  ...
  if (behavioralCertification?.passed !== true) {
    throw new Error(
      `${candidate.id}: patch report is not behaviorally certified for stable publication`,
    );
  }
  const evidence = report.candidates?.find(
    (entry) =>
      entry.module === candidate.package &&
      entry.upstreamVersion === candidate.version,
  );
  if (evidence === undefined)
    throw new Error(
      `${candidate.id}: patch certifier produced no exact candidate evidence`,
    );
  if (candidate.registry === "go") {
    if (
      evidence.material?.sum !== candidate.source.sum ||
      evidence.material?.goModSum !== candidate.source.goModSum ||
      withoutSha256Prefix(evidence.material?.zipDigest) !==
        candidate.source.zipSha256
    )
      throw new Error(
        `${candidate.id}: certified Go source does not match the discovered checksums`,
      );
  } else if (
    withoutSha256Prefix(evidence.material?.checksum) !==
      candidate.source.checksum ||
    withoutSha256Prefix(evidence.material?.archiveDigest) !==
      candidate.source.checksum
  )
    throw new Error(
      `${candidate.id}: certified crates.io source does not match the discovered checksum`,
    );
}

export async function verifyPreparedUpdateInvariant({
  directory,
  bundle,
  patchTreeDigest,
}) {
  if (
    !Buffer.from(await readFile(join(directory, "bundle.json"))).equals(
      Buffer.from(bundle),
    )
  ) {
...
export function candidateToolchainBlock(candidate, approvedGoVersion) {
  ...
  if (typeof candidate.source.requiredGoVersion !== "string")
    throw new Error(
      `${candidate.id}: unsupported toolchain evidence has no required Go version`,
    );
...
export async function assertCandidateSemanticSession(session, candidateId) {
  ...
  if (
    ...
    throw new Error(
      `${candidateId}: candidate application produced no supported semantic tree`,
    );
  }
}

export function selectCharmCandidateComposition(
  candidate,
  patchSets,
  tea,
  bubbles,
  ...
  const capabilityCandidate =
    ...
    candidate.package === bubbles &&
    candidate.capability === "bubbles-private-state" &&
    candidate.capabilityStrategy === "compile-conformance";
  if (
    !capabilityCandidate &&
    !patchSets.some(
      (entry) =>
        entry.name === candidate.package && entry.version === candidate.version,
    )
  ) {
    throw new Error(
      `${candidate.id}: exact candidate patch declaration is missing`,
    );
  }
  const latest = (module) =>
    patchSets
  ...
  const teaVersion =
    candidate.package === tea ? candidate.version : latest(tea);
  const bubblesVersion =
    candidate.package === bubbles ? candidate.version : latest(bubbles);
  if (teaVersion === undefined || bubblesVersion === undefined) {
    throw new Error(
      `${candidate.id}: no exact certified Charm companion exists for candidate-specific behavior`,
    );
  }
...
export function isSupportedCompileCapabilityCandidate(candidate) {
  ...
  if (candidate.frameworkId === "tview") {
    return (
      (candidate.package === "github.com/rivo/tview" &&
        candidate.capability === "tview-private-state") ||
      (candidate.package === "github.com/gdamore/tcell/v2" &&
        candidate.capability === "tcell-same-writer-marker")
    );
  }
  return (
    candidate.frameworkId === "charm" &&
    ["github.com/charmbracelet/bubbles", "charm.land/bubbles/v2"].includes(
      candidate.package,
    ) &&
    candidate.capability === "bubbles-private-state"
  );
...
export async function bindLocalTermwrightGoClient(
  moduleDir,
  env = process.env,
...
export async function certifyGoCandidateBehavior(candidate) {
  ...
  if (candidate.frameworkId === "tview") {
    if (
      !["github.com/rivo/tview", "github.com/gdamore/tcell/v2"].includes(
        candidate.package,
      )
    ) {
    ...
    await run("cp", [
      "-R",
      `${join(root, "packages/probe-tview/src/testing/fixture-app")}/.`,
      app,
    ]);
    await run(
      "go",
      ["mod", "edit", `-require=${candidate.package}@${candidate.version}`],
      env,
      app,
    );
    ...
    const compatibility = JSON.parse(
      await readFile(join(root, "compatibility/registry.json"), "utf8"),
    );
    const patchSets = compatibility.frameworks.find(
      (entry) => entry.id === "charm",
    )?.instrumentation?.patchSets;
    if (!Array.isArray(patchSets))
      throw new Error(
        `${candidate.id}: certified Charm patch declarations are missing`,
      );
    const { teaVersion, bubblesVersion } = selectCharmCandidateComposition(
      candidate,
      patchSets,
      tea,
      bubbles,
    ...
    await writeFile(
      join(app, "go.mod"),
      `module example.com/termwright-candidate\n\ngo 1.25\n\nrequire (\n\t${tea} ${teaVersion}\n\t${bubbles} ${bubblesVersion}\n)\n`,
    );
    await writeFile(join(app, "main.go"), source);
  } else
    throw new Error(
      `${candidate.id}: no candidate-specific Go behavioral profile`,
    );
  ...
  if (candidate.frameworkId === "tview") {
    frameworkVersion = (
      await run(
        "go",
        ["list", "-m", "-f", "{{.Version}}", "github.com/rivo/tview"],
        { ...env, GOWORK: "off" },
        app,
      )
    ).stdout.trim();
    const tcellVersion = (
      await run(
        "go",
        ["list", "-m", "-f", "{{.Version}}", "github.com/gdamore/tcell/v2"],
        { ...env, GOWORK: "off" },
        app,
      )
    ).stdout.trim();
  ...
  await run(
    "go",
    ["build", ...prepared.goArgs, "-o", binary, "."],
    prepared.env,
    app,
  );
  const driver = await import(
    pathToFileURL(join(root, "packages/driver/dist/index.js")).href
  );
  ...
  try {
    await session.waitForText(
      candidate.frameworkId === "tview" ? "readme.md" : "candidate",
    );
    ...
    if (candidate.frameworkId === "tview") {
      ...
      if (initial.semanticRevision === null)
        throw new Error(
          `${candidate.id}: initial semantic tree has no committed revision`,
        );
      ...
      if (
        ...
        throw new Error(
          `${candidate.id}: causal redraw did not publish a newer semantic tree`,
        );
      }
      await assertCandidateSemanticSession(session, candidate.id);
      ...
      if (
        !Number.isSafeInteger(state?.positionInSet) ||
        !Number.isSafeInteger(state?.setSize)
      ) {
        throw new Error(
          `${candidate.id}: Bubbles private spinner state was not observed through the compiled owned accessor contract`,
        );
      }
    }
  } finally {
  ...
  if (
    ...
    (candidate.frameworkId === "tview" && typeof frameworkVersion !== "string")
  ) {
    throw new Error(
      `${candidate.id}: executable framework resolution is incomplete`,
    );
  }
  if (
    !modules.some(
      (module) =>
        module.name === candidate.package &&
        module.version === candidate.version,
    )
  ) {
    throw new Error(
      `${candidate.id}: exact candidate was not on the executed instrumented path`,
    );
  }
  return {
    passed: true,
...
export async function certifyRustCandidateBehavior(candidate) {
  if (
    ...
    !["ratatui-core", "ratatui-widgets", "ratatui-crossterm"].includes(
      candidate.package,
    )
  )
    throw new Error(
      `${candidate.id}: no candidate-specific Rust behavioral profile`,
    );
  const compatibility = JSON.parse(
    await readFile(join(root, "compatibility/registry.json"), "utf8"),
  );
  const framework = compatibility.frameworks.find(
    (entry) => entry.id === "ratatui",
  );
  const currentVariant = framework?.instrumentation?.variants
    ?.slice()
    .sort((left, right) =>
      compareVersions(right.frameworkVersion, left.frameworkVersion),
    )[0];
  if (currentVariant === undefined)
    throw new Error(
      `${candidate.id}: no certified Ratatui framework variant exists`,
    );
  const versions = Object.fromEntries(
    currentVariant.modules.map((module) => [module.name, module.version]),
  );
...
async function main(argv) {
  ...
  if (
    ...
    throw new Error(
      "--registry, --candidate, --platform and --output are required",
    );
  }
  ...
  if (candidate === undefined)
    throw new Error(`candidate registry has no ${candidateId}`);
  const revision = process.env.GITHUB_SHA ?? "local-unpinned";
  await writeVerdict(
    output,
    candidate,
    "red",
    "Certification did not complete; inspect the job log.",
    revision,
  );
  const actualPlatform = certificationPlatform();
  if (expectedPlatform !== actualPlatform) {
    throw new Error(
      `${candidate.id}: expected ${expectedPlatform} certification but host is ${actualPlatform}`,
    );
  }
  if (initializeOnly) return;
  const toolchainBlock = candidateToolchainBlock(
    candidate,
    process.env.TERMWRIGHT_UPSTREAM_GO_VERSION ?? "unknown",
  );
  ...
  if (candidate.mode === "capability") {
    try {
      if (
        ...
        !isSupportedCompileCapabilityCandidate(candidate)
      ) {
        throw new Error(
          `${candidate.id}: unsupported capability/compile-conformance candidate`,
        );
      }
      ...
      await runPnpm(
        [
          ...
          "run",
          "conformance",
          "--require-no-skipped-areas",
        ],
        { ...process.env, TERMWRIGHT_REQUIRE_RATATUI: "1" },
      );
      await writeVerdict(
        output,
        candidate,
  ...
  if (candidate.mode === "hook") {
    try {
      if (candidate.registry === "npm") {
        if (candidate.source.closureComplete !== true)
          throw new Error(
            `${candidate.id}: production dependency closure is not fully pinned: ${JSON.stringify(candidate.source.unresolvedDependencyDeclarations ?? [])}`,
          );
        if (candidate.monitorDependencyClosure === true) {
          const current = await resolveNpmSource(
            { id: candidate.streamId, monitorDependencyClosure: true },
            candidate.source,
            { reuseSource: candidate.source },
          );
          if (current.closureDigest !== candidate.source.closureDigest)
            throw new Error(
              `${candidate.id}: npm dependency closure changed after discovery`,
            );
        }
        const probe =
          candidate.frameworkId === "ink"
        ...
        const material = await mkdtemp(
          join(tmpdir(), "termwright-npm-candidate-"),
        );
        ...
        if (!["exact-source", "runtime"].includes(candidate.hookStrategy)) {
          throw new Error(
            `${candidate.id}: hook candidate has no explicit certification strategy`,
          );
        }
        const profile =
          candidate.hookStrategy === "exact-source"
            ? await deriveHookInstrumentationProfile(
                candidate,
                archiveBytes,
                revision,
              )
        ...
        const previous = Object.fromEntries(
          Object.keys(certificationEnv).map((key) => [key, process.env[key]]),
        );
          ...
          if (candidate.hookStrategy === "exact-source") {
            const instrumentation = await import(
              pathToFileURL(
                join(
                  root,
                  `packages/probe-${candidate.frameworkId}/dist/instrumentation.js`,
                ),
              ).href
            );
            if (
              ...
              instrumentation.instrumentInkCore(
                "ink/build/ink.js",
                profile.sources.core,
              ) === undefined
            ) {
              throw new Error(
                `${candidate.id}: exact Ink transform anchors no longer apply`,
              );
            }
          }
        } finally {
        ...
        if (candidate.monitorDependencyClosure === true) {
          await verifyInstalledNpmClosure(
            candidate,
            join(root, `packages/probe-${candidate.frameworkId}`),
          );
        }
        if (candidate.frameworkId === "opentui")
          await run("bun", ["--version"], certificationEnv);
        ...
        await run("python", [
          ...
          "pip",
          "install",
          `${candidate.source.url}#sha256=${candidate.source.sha256}`,
        ]);
        const certificationEnv = {
          ...process.env,
          TERMWRIGHT_REQUIRE_RATATUI: "1",
        ...
        throw new Error(
          `${candidate.id}: unsupported hook registry ${candidate.registry}`,
        );
      }
      await writeVerdict(
        output,
  ...
  try {
    ...
    if (candidate.patch.status === "needs-patch") {
      const updateDirectory = await mkdtemp(
        join(tmpdir(), "termwright-candidate-update-"),
      );
      ...
      proposedRegistry = proposeCompatibilityUpdate(
      ...
      const registryPath = join(root, "compatibility/registry.json");
      const manifest = JSON.parse(
        await readFile(join(root, candidate.patch.path), "utf8"),
      );
      proposedRegistry = proposeCompatibilityUpdate(
        JSON.parse(await readFile(registryPath, "utf8")),
        candidate,
      ...
      throw new Error(
        `${candidate.id}: unsupported patch state ${candidate.patch.status}`,
      );
    }
    const evidenceDirectory = await mkdtemp(
      join(tmpdir(), "termwright-patch-evidence-"),
    );
    await run("node", [
      "scripts/certify-upstream-patches.mjs",
      "--ecosystem",
      candidate.ecosystem,
      "--source-revision",
      revision,
    ...
    verifyCandidateEvidence(
      ...
      JSON.parse(
        await readFile(
          join(evidenceDirectory, "candidate-report.json"),
          "utf8",
        ),
      ),
      behavioral,
    );
    proposedRegistry = recordExecutableVariant(
      proposedRegistry,
      candidate,
      behavioral.resolution,
    );
    await writeFile(
      join(root, "compatibility/registry.json"),
      canonicalJson(proposedRegistry),
    );
    await runPnpm(["run", "test:compatibility"]);
    await runPnpm(
      [
    ...
    if (preparedUpdate !== undefined)
      await verifyPreparedUpdateInvariant(preparedUpdate);
    await writeVerdict(
      output,
      candidate,
      "green",
      "Exact source, deterministic patching, candidate-specific real-process behavior, and full conformance passed.",
      revision,
      behavioral.resolution,
    );
  } catch (error) {
