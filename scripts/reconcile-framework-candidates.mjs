...
async function verdictFiles(directory) {
  ...
  const visit = async (path) => {
    for (const entry of await readdir(path, { withFileTypes: true })) {
      ...
      else if (entry.name.endsWith('.json') && entry.name.startsWith('verdict-'))
        found.push(target);
    }
  };
...
export function generatedUpdateName(candidate) {
  ...
  if (candidate.mode === 'hook' && candidate.hookStrategy === 'runtime')
    return `candidate-update-runtime-${suffix}`;
  if (candidate.mode === 'hook' && candidate.hookStrategy === 'exact-source')
    return `candidate-update-hook-${suffix}`;
  if (candidate.mode === 'patch' && candidate.patch.status === 'needs-patch')
    return `candidate-update-${suffix}`;
  return null;
}
...
export function reconcile(registry, ledger, verdicts, context = {}) {
  ...
  for (const verdict of verdicts) {
    ...
    if (candidate === undefined || candidate.candidateDigest !== verdict.candidateDigest)
      throw new Error(`untrusted or stale verdict for ${verdict.candidateId}`);
    if (byCandidate.has(verdict.candidateId))
      throw new Error(`duplicate verdict for ${verdict.candidateId}`);
    if (!['green', 'red'].includes(verdict.state))
      throw new Error(`invalid verdict state for ${verdict.candidateId}`);
    if (
  ...
  for (const candidate of registry.candidates) {
    ...
    if (verdict?.state === 'green') {
      ...
      else if (entries[existing].candidateDigest !== candidate.candidateDigest)
      ...
      if (typeof context.runUrl !== 'string' || !/^https:\/\//u.test(context.runUrl))
        throw new Error(`${candidate.id}: failure issue requires an authenticated source run URL`);
      if (
        typeof context.owner !== 'string' ||
        !/^[A-Za-z0-9](?:[A-Za-z0-9-]{0,38})$/u.test(context.owner)
      )
        throw new Error(`${candidate.id}: failure issue requires a repository owner`);
      issues.push({
        key: candidate.id,
        owner: context.owner,
        ...
        body: [
          ...
          ...(candidate.mode === 'capability'
            ? [
                ...
                `Capability strategy: \`${candidate.capabilityStrategy}\``,
              ]
            : []),
      ...
      if (verdict?.state === 'red') {
        if (
          !Number.isSafeInteger(candidate.certificationRevision) ||
          candidate.certificationRevision < 1
        ) {
  ...
  return {
    ...
    plan: {
      schemaVersion: 1,
      green: registry.candidates
        .filter((entry) => byCandidate.get(entry.id)?.state === 'green')
        .map((entry) => entry.id),
      issues,
    },
  };
...
export async function verifyGeneratedUpdate({
  candidate,
  verdict,
  updateDirectory,
  expectedRevision,
  ...
  if (verdict.sourceRevision !== expectedRevision || metadata.sourceRevision !== expectedRevision)
    throw new Error(`${candidate.id}: stale source revision in generated update`);
  ...
  if ((await digestTree(join(updateDirectory, 'patch'))) !== metadata.patchTreeDigest)
    throw new Error(`${candidate.id}: generated patch digest mismatch`);
  return metadata;
}

export async function verifyGeneratedHookProfile({
  candidate,
  verdict,
  updateDirectory,
  expectedRevision,
  ...
  if (metadata.profileDigest !== digest)
    throw new Error(`${candidate.id}: generated hook profile digest mismatch`);
  return metadata.profile;
}

export async function verifyGeneratedRuntimeProfile({
  candidate,
  verdict,
  updateDirectory,
  expectedRevision,
  ...
  if (
    ...
    throw new Error(
      `${candidate.id}: generated runtime profile is not bound to the green candidate`,
    );
  const digest = `sha256:${createHash('sha256').update(canonicalJson(metadata.profile)).digest('hex')}`;
  if (metadata.profileDigest !== digest)
    throw new Error(`${candidate.id}: generated runtime profile digest mismatch`);
  return metadata.profile;
}
...
export function addCertifiedRuntimeProfile(document, candidate, profile) {
  ...
  else if (canonicalJson(existing) !== canonicalJson(profile))
    throw new Error(`${candidate.id}: certified runtime profile is immutable`);
...
export function recordVerifiedFrameworkVersion(registry, candidate) {
  ...
  if (framework === undefined)
    throw new Error(`${candidate.id}: compatibility framework row is missing`);
  if (
    typeof framework.certification?.adapterVersion !== 'string' ||
    framework.certification.adapterVersion.length === 0
  ) {
    throw new Error(
      `${candidate.id}: compatibility framework certification adapter version is missing`,
    );
  }
  if (
    typeof framework.frameworkPackage !== 'string' ||
    candidate.package !== framework.frameworkPackage
  ) {
    throw new Error(
      `${candidate.id}: candidate package does not match the compatibility framework package`,
    );
  }
  const next = structuredClone(framework);
  if (!next.versions.verified.includes(candidate.version))
    next.versions.verified.push(candidate.version);
  next.versions.verified.sort(compareVersions);
  if (next.versions.policy === 'exact')
    next.versions.declared = next.versions.verified.join(' or ');
  ...
  if (next.certification.strategy !== 'native-hook') {
    ...
    if (!Array.isArray(variants))
      throw new Error(`${candidate.id}: exact framework variants are missing`);
...
async function main(argv) {
  ...
  if (registryPath === undefined || verdictDirectory === undefined)
    throw new Error('--registry and --verdicts are required');
  ...
  const verdicts = await Promise.all(
    (await verdictFiles(verdictDirectory)).map(async (path) =>
  ...
  const expectedUpdateNames = registry.candidates
    .filter(
      (candidate) =>
        verdicts.find((entry) => entry.candidateId === candidate.id)?.state === 'green',
    )
  ...
  if ([...updates.keys()].sort().join('\0') !== expectedUpdateNames.join('\0')) {
    throw new Error(
      'generated candidate update artifact set does not match the exact green candidates',
    );
  }
  for (const candidate of registry.candidates) {
    ...
    if (candidate.mode === 'capability') {
      if (
        ...
        verdict.executableResolution === null ||
        typeof verdict.executableResolution !== 'object'
      ) {
        throw new Error(
          `${candidate.id}: green capability verdict has no compile-conformance executable resolution`,
        );
      }
      compatibility = recordExecutableVariant(
        compatibility,
        candidate,
        verdict.executableResolution,
      );
      continue;
    }
    if (candidate.mode === 'hook') {
      if (candidate.frameworkId === 'textual') {
        if (candidate.registry !== 'pypi' || candidate.hookStrategy !== 'runtime')
          throw new Error(
            `${candidate.id}: green Textual verdict does not describe runtime capability certification`,
          );
        continue;
      }
      if (candidate.hookStrategy === 'runtime') {
        ...
        if (matched === undefined)
          throw new Error(
            `${candidate.id}: green runtime-hook verdict has no generated capability profile`,
          );
      ...
      if (matched === undefined)
        throw new Error(
          `${candidate.id}: green hook verdict has no generated instrumentation profile`,
        );
      ...
      const existing = document.profiles.find((entry) => entry.version === candidate.version);
      if (existing === undefined) document.profiles.push(profile);
      else if (!sameHookProfile(existing, profile))
        throw new Error(`${candidate.id}: certified hook profile is immutable`);
      document.profiles.sort((left, right) => compareVersions(left.version, right.version));
      await writeFile(profilePath, canonicalJson(document));
    ...
    if (candidate.patch.status === 'needs-patch') {
      const matched = updates.get(generatedUpdateName(candidate));
      if (matched === undefined)
        throw new Error(`${candidate.id}: green verdict has no generated update bundle`);
