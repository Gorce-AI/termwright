...
export function validateCandidateAssessments(document, streamIds) {
  if (
    ...
    document.streams === null ||
    typeof document.streams !== 'object' ||
  ...
  for (const [streamId, entries] of Object.entries(document.streams)) {
    if (allowed !== null && !allowed.has(streamId))
      throw new Error(`candidate assessments contain unknown stream ${streamId}`);
    if (!Array.isArray(entries))
      throw new Error(`${streamId}: candidate assessments must be an array`);
    ...
    for (const entry of entries) {
      ...
      if (versions.has(entry.version))
        throw new Error(`${streamId}: duplicate candidate assessment for ${entry.version}`);
...
function matchesCatalogSource(catalogSource, storedSource) {
  ...
  return (
    !Array.isArray(storedSource) &&
    Object.entries(catalogSource).every(([key, value]) =>
      matchesCatalogSource(value, storedSource[key]),
    )
  );
}
...
function validateSource(entry, stream) {
  if (stream.registry === 'go') {
    if (typeof entry.source?.sum !== 'string' || !entry.source.sum.startsWith('h1:'))
      throw new Error(`${stream.id}@${entry.version}: missing Go module sum`);
    if (typeof entry.source?.goModSum !== 'string' || !entry.source.goModSum.startsWith('h1:'))
      throw new Error(`${stream.id}@${entry.version}: missing Go module go.mod sum`);
    if (typeof entry.source?.zipSha256 !== 'string' || !SHA256.test(entry.source.zipSha256))
      throw new Error(`${stream.id}@${entry.version}: missing zip sha256`);
    const unsupported = entry.source?.toolchainSupported === false;
    const required =
      typeof entry.source?.requiredGoVersion === 'string' &&
      /^\d+(?:\.\d+){1,2}$/u.test(entry.source.requiredGoVersion);
    if (unsupported !== required)
      throw new Error(
        `${stream.id}@${entry.version}: incomplete unsupported Go toolchain evidence`,
      );
  } else if (stream.registry === 'npm') {
    if (
      typeof entry.source?.integrity !== 'string' ||
      !entry.source.integrity.startsWith('sha512-')
    )
      throw new Error(`${stream.id}@${entry.version}: missing npm integrity`);
    if (typeof entry.source?.shasum !== 'string' || !SHA1.test(entry.source.shasum))
      throw new Error(`${stream.id}@${entry.version}: missing npm shasum`);
    if (typeof entry.source?.tarballSha256 !== 'string' || !SHA256.test(entry.source.tarballSha256))
      throw new Error(`${stream.id}@${entry.version}: missing npm tarball sha256`);
    if (stream.monitorDependencyClosure === true) {
      if (
        entry.source?.closureComplete !== true ||
        !Array.isArray(entry.source?.dependencyRoots) ||
        !Array.isArray(entry.source?.dependencyClosure)
      )
        throw new Error(`${stream.id}@${entry.version}: npm dependency closure is incomplete`);
      if (
        typeof entry.source?.closureDigest !== 'string' ||
        !/^sha256:[0-9a-f]{64}$/u.test(entry.source.closureDigest)
      )
      ...
      if (
        ...
        throw new Error(
          `${stream.id}@${entry.version}: npm dependency closure digest does not match the graph`,
        );
    }
    for (const dependency of entry.source?.dependencyClosure ?? []) {
      if (
        ...
        throw new Error(
          `${stream.id}@${entry.version}: dependency closure is not fully checksum-bound`,
        );
    }
    if (stream.monitorDependencyClosure === true) {
      const nodes = new Set();
      for (const dependency of entry.source.dependencyClosure) {
        ...
        if (nodes.has(key))
          throw new Error(`${stream.id}@${entry.version}: duplicate npm closure node ${key}`);
        nodes.add(key);
        if (!Array.isArray(dependency.dependencies))
          throw new Error(
            `${stream.id}@${entry.version}: npm closure node ${key} has no exact edges`,
          );
      }
      for (const edge of [
        ...entry.source.dependencyRoots,
        ...entry.source.dependencyClosure.flatMap((dependency) => dependency.dependencies),
      ]) {
        if (
    ...
    if (typeof entry.source?.sha256 !== 'string' || !SHA256.test(entry.source.sha256))
      throw new Error(`${stream.id}@${entry.version}: missing PyPI file sha256`);
    if (
      !Array.isArray(entry.source?.files) ||
      entry.source.files.length === 0 ||
      entry.source.files.some(
        (file) => typeof file.sha256 !== 'string' || !SHA256.test(file.sha256),
      )
    )
...
export async function selectCandidates({
  ...
  config,
  ledger,
  assessments = { schemaVersion: 1, streams: {} },
  catalogs,
  maximum = config.maxCandidatesPerRun,
  sourceResolver,
  streamId,
}) {
  if (!Number.isSafeInteger(maximum) || maximum < 1 || maximum > 32)
    throw new Error('maximum must be between 1 and 32');
  ...
  const pending = [];
  const selectedStreams = config.streams.filter(
    (stream) => streamId === undefined || stream.id === streamId,
  );
  for (const stream of [...selectedStreams].sort((a, b) => a.id.localeCompare(b.id))) {
    ...
    if (integrationMode === 'hook' && !['exact-source', 'runtime'].includes(stream.hookStrategy)) {
      throw new Error(
        `${stream.id}: hook mode requires an explicit exact-source or runtime strategy`,
      );
    }
    if (
      integrationMode === 'capability' &&
      (stream.capabilityStrategy !== 'compile-conformance' ||
        typeof stream.capability !== 'string' ||
        !/^[a-z][a-z0-9-]*$/u.test(stream.capability))
    ) {
      throw new Error(
        `${stream.id}: capability mode requires a normalized capability and compile-conformance strategy`,
      );
    }
    if (!['patch', 'hook', 'capability'].includes(integrationMode)) {
      throw new Error(
        `${stream.id}: unsupported candidate integration mode ${String(integrationMode)}`,
      );
    }
    const seen = new Map(
      (ledger.streams?.[stream.id] ?? []).map((entry) => [entry.version, entry]),
    );
    const assessed = new Map(
      (assessments.streams[stream.id] ?? []).map((entry) => [entry.version, entry]),
    );
    ...
    for (const entry of catalogs[stream.id] ?? []) {
      ...
      if (
        ...
        (stream.major !== undefined && parsed.major !== stream.major)
      )
        continue;
      if (compareVersions(entry.version, stream.minimumVersion) < 0 || entry.yanked === true)
        continue;
      ...
      if (recorded !== undefined) {
        ...
        resolvedSource =
          sourceResolver === undefined
            ? entry.source
            : await sourceResolver(stream, entry.version, entry.source, recorded.source);
      ...
      if (!Number.isFinite(Date.parse(entry.publishedAt)))
        throw new Error(`${stream.id}@${entry.version}: invalid publishedAt`);
      const patchMode = integrationMode === 'patch';
      const manifestPath = patchMode
        ? join(rootDir, stream.patchRoot, entry.version, 'manifest.json')
        : null;
      const ready =
        patchMode &&
      ...
      const candidate = {
        ...
        monitorDependencyClosure:
          stream.registry === 'npm' && stream.monitorDependencyClosure === true,
  ...
  const comparePending = (a, b) =>
    Date.parse(a.candidate.publishedAt) - Date.parse(b.candidate.publishedAt) ||
    a.candidate.streamId.localeCompare(b.candidate.streamId) ||
    compareVersions(a.candidate.version, b.candidate.version);
  ...
  while (scheduled.length < maximum && scheduled.length < pending.length) {
    const round = [...queues.values()]
      ...
      .sort((a, b) => comparePending(a[0], b[0]));
  ...
  for (const { candidate, stream, sourceResolved, reuseSource } of scheduled) {
    const source =
      sourceResolver === undefined || sourceResolved
        ? candidate.source
        : await sourceResolver(stream, candidate.version, candidate.source, reuseSource);
...
async function goCatalog(stream) {
  ...
  for (const version of versions) {
    ...
    if (
      ...
      parsed.prerelease !== null ||
      (stream.major !== undefined && parsed.major !== stream.major) ||
      compareVersions(version, stream.minimumVersion) < 0
    )
      continue;
    const infoResponse = await fetch(
      `https://proxy.golang.org/${stream.package}/@v/${version}.info`,
    );
    if (!infoResponse.ok)
      throw new Error(`${stream.id}@${version}: Go proxy info failed with ${infoResponse.status}`);
...
export function parseGoDownloadResult(moduleVersion, stdout) {
  ...
  if (typeof downloaded.Error === 'string' && downloaded.Error.length > 0) {
    const unsupported =
      /requires go >= ([0-9]+(?:\.[0-9]+){1,2}) \(running go [^;]+; GOTOOLCHAIN=local\)$/u.exec(
        downloaded.Error,
      );
    if (
      unsupported !== null &&
      typeof downloaded.Sum === 'string' &&
      typeof downloaded.GoModSum === 'string' &&
      typeof downloaded.Zip === 'string'
    ) {
...
async function resolveSource(stream, version, source, recordedSource) {
  if (stream.registry === 'npm')
    return resolveNpmSource(stream, source, { reuseSource: recordedSource });
...
async function cratesCatalog(stream) {
  const response = await fetch(
    `https://crates.io/api/v1/crates/${encodeURIComponent(stream.package)}/versions`,
...
export async function pypiCatalog(stream, fetchImpl = fetch) {
  const response = await fetchImpl(
    `https://pypi.org/pypi/${encodeURIComponent(stream.package)}/json`,
  );
  ...
  return Object.entries(body.releases ?? {}).flatMap(([version, releaseFiles]) => {
    const usable = [...releaseFiles]
      ...
      .sort(
        (a, b) =>
          Number(b.packagetype === 'sdist') - Number(a.packagetype === 'sdist') ||
...
export async function downloadVerifiedNpmTarball(source, fetchImpl = fetch) {
  if (
    ...
    typeof source?.shasum !== 'string'
  )
    throw new Error('npm release lacks dist evidence');
  const [algorithm, expectedIntegrity] = source.integrity.split('-', 2);
  if (algorithm !== 'sha512' || expectedIntegrity === undefined)
    throw new Error('npm release does not publish sha512 integrity');
  const bytes = await verifiedDownload(
    source.tarball,
    expectedIntegrity,
    'sha512',
    fetchImpl,
    fetchImpl === fetch ? liveNpmArtifacts : null,
  );
  if (createHash('sha1').update(bytes).digest('hex') !== source.shasum)
    throw new Error('npm shasum does not match tarball');
  const sha256 = createHash('sha256').update(bytes).digest('hex');
  if (source.tarballSha256 !== undefined && source.tarballSha256 !== sha256)
    throw new Error('npm tarball sha256 does not match discovered evidence');
  return bytes;
}
...
function compareNpmVersions(left, right) {
  ...
  if (a === null || b === null)
    throw new Error(`invalid npm semantic version: ${a === null ? left : right}`);
  return (
    a.major - b.major ||
    a.minor - b.minor ||
    a.patch - b.patch ||
...
function partialBounds(token) {
  const match =
    /^(?:v)?(\d+|[xX*])(?:\.(\d+|[xX*]))?(?:\.(\d+|[xX*]))?(?:-([0-9A-Za-z.-]+))?$/u.exec(token);
  if (match === null) return null;
...
function tokenPredicates(token) {
  ...
  if (value.startsWith('^') || value.startsWith('~')) {
    ...
    if (bounds === null || bounds.any === true)
      throw new Error(`unsupported npm range token ${token}`);
    const parsed = npmVersion(bounds.lower);
    let upper;
    if (kind === '~')
      upper =
        bounds.precision < 2 ? `${parsed.major + 1}.0.0` : `${parsed.major}.${parsed.minor + 1}.0`;
  ...
  if (operator !== '') {
    ...
    if (bounds.exact === true || ['>', '>='].includes(operator))
      return [comparator(operator, bounds.lower)];
...
function satisfiesNpmRange(version, range) {
  ...
  return range.split(/\s*\|\|\s*/u).some((alternative) => {
    ...
    const tokens =
      hyphen === null
        ? alternative.trim().split(/\s+/u).filter(Boolean)
        : [`>=${hyphen[1]}`, `<=${hyphen[2]}`];
    ...
    if (
      ...
      !tokens.some(
        (token) =>
          token.includes('-') && token.includes(`${parsed.major}.${parsed.minor}.${parsed.patch}`),
      )
    )
      return false;
    return true;
...
function dependencyDeclarations(metadata) {
  ...
  if (bundled === true || (Array.isArray(bundled) && bundled.length > 0))
    throw new Error(
      `${metadata.name ?? 'npm package'}: bundled production dependencies cannot be independently checksum-bound`,
    );
  ...
  for (const [type, values] of [
    ...
    for (const [name, requested] of Object.entries(values)) {
      ...
      if (type === 'dependency' && Object.hasOwn(metadata.optionalDependencies ?? {}, name))
        continue;
  ...
  return [...declarations.values()].sort(
    (a, b) =>
      ...
      String(a.requested).localeCompare(String(b.requested)),
  );
}

function parseNpmAlias(declaredName, requested) {
  if (typeof requested !== 'string' || requested.length === 0)
    throw new Error(`${declaredName}: dependency selector must be a non-empty string`);
...
export async function resolveNpmDependencyClosure(
  ...
  {
    fetchImpl = fetch,
    packuments = fetchImpl === fetch ? liveNpmPackuments : new Map(),
    artifacts = fetchImpl === fetch ? liveNpmArtifacts : new Map(),
    reuseClosure = [],
  } = {},
) {
  const reusable = new Map(
    (reuseClosure ?? []).map((node) => [`${node.name}@${node.version}`, node]),
  );
  const nodes = new Map();
    ...
    if (!packuments.has(name)) {
      packuments.set(
        ...
        (async () => {
          ...
          if (!response.ok)
            throw new Error(
              `npm dependency ${name}: registry metadata failed with ${response.status}`,
            );
  ...
  const resolveDeclaration = async (declaration) => {
    ...
    if (version === undefined || packument.versions?.[version] === undefined)
      throw new Error(
        `npm dependency ${declaration.name}@${range}: no exact registry version satisfies the selector`,
      );
      ...
      resolving.set(
        ...
        (async () => {
          ...
          if (metadata.name !== undefined && metadata.name !== packageName)
            throw new Error(`${key}: registry metadata returned another package name`);
          ...
          if (
            ...
            throw new Error(`${key}: dependency lacks sha512 integrity or tarball URL`);
          ...
          if (
            ...
            createHash('sha1').update(bytes).digest('hex') !== metadata.dist.shasum
          )
          ...
          node.dependencies.sort(
            (a, b) =>
              ...
              compareNpmVersions(a.version, b.version),
          );
          return node;
        })(),
      );
  ...
  const dependencyClosure = [...nodes.values()].sort(
    (a, b) => a.name.localeCompare(b.name) || compareNpmVersions(a.version, b.version),
  );
  return {
...
export async function resolveNpmSource(stream, source, { fetchImpl = fetch, reuseSource } = {}) {
  if (
    ...
    throw new Error(`${stream.id}: npm release lacks dist evidence`);
  const [algorithm, expectedIntegrity] = source.integrity.split('-', 2);
  if (algorithm !== 'sha512' || expectedIntegrity === undefined)
    throw new Error(`${stream.id}: npm release does not publish sha512 integrity`);
  const canReuseRoot =
  ...
  return {
    ...source,
    tarballSha256: canReuseRoot
      ? reuseSource.tarballSha256
      : createHash('sha256').update(bytes).digest('hex'),
    ...closure,
  };
}
...
async function resolvePypiSource(stream, source) {
  if (typeof source.url !== 'string' || typeof source.sha256 !== 'string')
    throw new Error(`${stream.id}: PyPI release lacks a selected artifact`);
...
async function main(argv) {
  ...
  const catalogs =
    catalogPath === undefined
      ? await liveCatalogs(config)
      : JSON.parse(await readFile(catalogPath, 'utf8')).streams;
