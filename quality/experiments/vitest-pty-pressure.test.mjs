...
const telemetry =
  telemetryDirectory === undefined ? undefined : join(telemetryDirectory, `${source}.jsonl`);
...
test.concurrent.for(Array.from({ length: cases }, (_, index) => index))(
  'PTY pressure %i',
  async (index, context) => {
    ...
    const exitReady = new Promise((resolve) => {
      exitSubscription = pty.onExit((status) => {
        ...
        if (!output.includes(doneOutput)) {
          rejectOutput(
            new Error(
              `PTY ${index} exited before DONE (${String(status.code)}): ${JSON.stringify(output.slice(-512))}`,
            ),
          );
        }
        resolve(status);
        ...
        try {
          ...
          if (startedRecorded) {
            ...
            record({
              ...
              readyObserved: output.includes(readyOutput),
              releaseSent,
              doneObserved: output.includes(doneOutput),
              exited,
            });
          }
        } finally {
...
function record(value) {
  ...
  appendFileSync(
    telemetry,
    `${JSON.stringify({ ...value, source, pid: process.pid, threadId, timeMs: performance.timeOrigin + performance.now(), node: process.version, platform: process.platform, arch: process.arch })}\n`,
    'utf8',
  );
}
...
async function workerRendezvous(directory, workers) {
  if (!Number.isSafeInteger(workers) || workers < 1)
    throw new Error('invalid worker rendezvous width');
  await new Promise((resolve, reject) => {
    ...
    const deadline = setTimeout(
      () => finish(new Error(`worker rendezvous reached fewer than ${workers} files`)),
      15_000,
    );
