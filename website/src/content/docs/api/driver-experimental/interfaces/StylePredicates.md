---
title: "Interface: StylePredicates"
editUrl: false
pagefind: false
---

[**@termwright/driver/experimental**](../../)

***

[@termwright/driver/experimental](../../) / StylePredicates

# Interface: StylePredicates

Defined in: [selectors.ts:44](https://github.com/Gorce-AI/termwright/blob/main/packages/driver/src/selectors.ts#L44)

Style predicates for generic grid matching.

## Properties

### attributes?

> `readonly` `optional` **attributes?**: `Readonly`\<`Partial`\<`CellAttributes`\>\>

Defined in: [selectors.ts:47](https://github.com/Gorce-AI/termwright/blob/main/packages/driver/src/selectors.ts#L47)

***

### bg?

> `readonly` `optional` **bg?**: `string`

Defined in: [selectors.ts:46](https://github.com/Gorce-AI/termwright/blob/main/packages/driver/src/selectors.ts#L46)

***

### fg?

> `readonly` `optional` **fg?**: `string`

Defined in: [selectors.ts:45](https://github.com/Gorce-AI/termwright/blob/main/packages/driver/src/selectors.ts#L45)
