...
fn the_ladder_matches_the_reference() {
    ...
    let levels: Vec<String> =
        constants["logLevels"].as_array().unwrap().iter().map(|v| v.as_str().unwrap().to_owned()).collect();
    assert_eq!(levels, LOG_LEVELS);
    assert_eq!(constants["maxLogAttrs"].as_u64().unwrap() as usize, MAX_LOG_ATTRS);
...
fn a_built_record_validates() {
    ...
    record.seq = 1;
    record.validate(&DEFAULT_LIMITS).expect("a hand-built record was rejected");
}
...
fn a_non_finite_attribute_degrades_to_text_rather_than_breaking_the_record() {
    ...
    record.ts = 1;
    record.seq = 0;
    record.validate(&DEFAULT_LIMITS).expect("a degraded attribute broke the record");
}
...
fn going_over_budget_drops_locally_and_leaves_a_gap() {
    ...
    sorted.dedup();
    assert_eq!(seqs, sorted, "sequence numbers must not repeat or go backwards");
    assert!(
...
fn an_oversized_record_is_dropped_not_sent() {
    ...
    assert_eq!(records[0]["seq"], 2, "the dropped record still consumed its number");
