...
def test_observation_vectors_preserve_unknown_and_half_open_geometry():
    ...
    assert {example["status"] for example in vectors["examples"]} == set(vectors["statuses"])
    assert vectors["examples"][1]["evidence"]["strength"] == "authoritative"
    assert vectors["examples"][2]["reason"] == "awaiting-revision-pair"
