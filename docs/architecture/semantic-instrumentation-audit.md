...
2. **Inherited socketpair is unavailable through @lydell/node-pty 1.1.0** (no
   stdio/fd options). Path+token rendezvous survives _by necessity_; a
...
3. **OpenTUI writes frames from a native Zig thread** (`useThread=true`
   ...
   _primary_ runtime for OpenTUI (bun:ffi), not a fallback. (opentui.md)
...
5. **All four current adapters share one defect the spec forbids**: an
   unrecognized widget is _dropped_ and its children reparented, instead of
...
under backpressure _is_ the existing `get-tree` resync with a new
...
- **D3 amended**: wholesale node replacement already expresses retraction
  ...
  → full snapshot". Partial node patches remain a _costed future option_ to
